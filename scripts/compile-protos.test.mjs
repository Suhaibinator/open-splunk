...
const sanitizedGeneratedTypeScript = [
...
writeFileSync(path.join(tsDirectory, "contract.ts"), generatedTypeScript);
for (const relativePath of [
  ...
  writeFileSync(destination, generatedTypeScript);
}
`,
  );
}
...
test("compile-protos replaces generated files while preserving handwritten files", async (t) => {
  ...
  assert.equal(
    ...
    sanitizedGeneratedTypeScript,
  );
  assert.equal(
    await readFile(
      path.join(fixture.fixture, "gen", "ts", "google", "protobuf", "duration.ts"),
      "utf8",
    ),
    sanitizedGeneratedTypeScript,
  );
...
test("compile-protos preserves published outputs when TypeScript lint-header removal fails", async (t) => {
  if (process.platform === "win32") {
    t.skip("the protobuf compiler entrypoint is a bash script");
    return;
  }

  const fixture = await createFixture(t, "open-splunk-proto-lint-header-failure-");
  await installSuccessfulBuf(fixture);
  await executable(
    path.join(fixture.fakeBin, "awk"),
    "#!/usr/bin/env bash\nexit 71\n",
  );
  ...
  assert.match(result.stderr, /could not remove generated TypeScript lint header/);
  await assertOldOutputs(fixture);
});
