...
export const SPL_PIPELINE_COMMANDS = [
  ...
  },
  {
    ...
    detail: 'Filter with comparisons, direct isnull/isnotnull predicates, match(value, "regex") substring predicates using a literal bounded RE2 pattern, like(value, "pattern") whole-string predicates using a literal bounded wildcard pattern, now() as the fixed search-start Unix second, relative_time(time, "specifier") as nullable Unix seconds after a bounded literal offset-and-snap program in the effective IANA search timezone, strftime(time, "format") in that timezone, or strptime(text, "format") as nullable Unix seconds.',
  },
  {
    name: "eval",
