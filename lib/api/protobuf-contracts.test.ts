...
test("every protobuf HTTP route round-trips generated TypeScript messages against its wire fixture", () => {
  ...
  assert.equal(routeFixture.routes.length, 70);
  assert.equal(new Set(routeFixture.routes.map((route) => route.path)).size, 70);
...
test("generated knowledge snapshots match the shared Go deterministic wire and digest golden", () => {
  ...
  assert.equal(fixture.digestDomain, "open-splunk-knowledge-snapshot-format-1\0");
