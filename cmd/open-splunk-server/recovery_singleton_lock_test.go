...
func TestRecoveryCommandsShareRuntimeSingletonAcrossProcesses(t *testing.T) {
	for _, configuration := range []string{"environment", "cli override", "exact path spelling"} {
		t.Run(configuration, func(t *testing.T) {
			...
			t.Setenv("OPEN_SPLUNK_SERVER_LOCK_FILE", lockPath)
			...
			command.Env = []string{
				...
				"OPEN_SPLUNK_SERVER_LOCK_FILE=" + environmentPath,
				"TMPDIR=" + t.TempDir(),
			}
			...
			// A different control path prevents the database sidecar from masking
			// a split deployment-wide lock. Missing state also proves lock failure
			// precedes recovery reads, publication, and network activity.
			...
			commands := map[string]func() error{
				"backup-control-plane": func() error {
					return runBackupControlPlane(t.Context(), controlbackup.CreateOptions{
						DatabasePath: databasePath, Destination: filepath.Join(missing, "bundle"),
				...
				"restore-control-plane": func() error {
					return runRestoreControlPlane(t.Context(), controlbackup.RestoreOptions{
						DatabasePath: databasePath, Source: filepath.Join(missing, "bundle"),
					}, release, acquireHostServerLock)
				},
				"backup-deployment-recovery-set": func() error {
					return runBackupDeploymentRecoverySetWithDependencies(t.Context(), deploymentRecoveryBackupOptions{
						DatabasePath: databasePath,
					}, release, defaultDeploymentRecoveryDependencies())
				},
				"restore-deployment-recovery-set": func() error {
					return runRestoreDeploymentRecoverySetWithDependencies(t.Context(), deploymentRecoveryRestoreOptions{
						DatabasePath: databasePath,
					}, release, defaultDeploymentRecoveryDependencies())
				},
				"reconcile-deployment-recovery-marker": func() error {
					return runDeploymentRecoveryMarkerReconcile(t.Context(), validDeploymentRecoveryMarkerReconcileOptions(), defaultDeploymentRecoveryMarkerReconcileDependencies())
				},
			}
			for name, run := range commands {
				t.Run(name, func(t *testing.T) {
					if err := run(); !errors.Is(err, errServerAlreadyRunning) || !strings.Contains(err.Error(), lockPath) {
...
func TestRecoverySingletonProcessHelper(t *testing.T) {
	...
	config, err := parseRuntimeOptions(flags, arguments, runtimeEnvironment{lookup: os.LookupEnv, unset: os.Unsetenv})
	if err != nil {
		t.Fatal(err)
	}
