...
exact-field occurrence-count, true-only conditional-count, exact-field
numeric-sum, exact-field numeric-average, exact-field mixed-type
minimum/maximum, and exact-field chronological earliest/latest subsets are
...
History is bounded by configurable age and row-count limits in SQLite. The UI should expose Recent Searches and a full History view with Open in Search, Run Again, Save, and Delete actions. “Open” restores the original query and time-range expression. “Run Again” sends only the owner-scoped history ID; the server loads a detached retained snapshot, reuses its original SPL/app/time intent and effective index scope, reauthorizes the app and indexes, and creates a distinct job with one-hop history-rerun provenance. When a failed pre-planning attempt has no effective scope, its stored requested scope is the fallback. The new job resolves relative time against the current clock and captures fresh visibility and index-time cutoffs; it never reuses the old absolute range, compiler state, generated SQL, or result snapshot. If a formerly valid mixed relative/absolute time intent is no longer executable against the current clock, admission returns a conflict without creating a job. A source deleted before the scoped lookup is absent, while deletion after the lookup does not invalidate the detached admission or its durable provenance. History never stores generated SQL as the reusable source of truth.
