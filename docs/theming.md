...
that file `@import`s the whole layer in cascade order. Only the token files are
meant to carry a literal. A retheme should be an edit to `app/styles`, not a
search across 7,600 lines of rules.
...
`app/styles/index.css` is the whole load order, written down in CSS rather than
implied by JavaScript import order or by the bundler's chunking:
...
| 3 | `app/globals.css` | the shared layer: primitives (`.button`, `.status`, `.badge`, `.table`, `.drawer`, `.modal-card`, `.wordmark`), the product chrome, and the rules for every page that has no file of its own |
| 4 | `app/analytics/analytics.css` | the search-analytics console, prefix `analytics-` |
| 5 | `app/dashboards/operations-dashboard.css` | the operations dashboard and the backend dashboard manager, prefix `operations-` |
| 6 | `app/reports/reports.css` | the reports library and the saved-search console, prefix `reports-` |
| 7 | `app/search-workspace/components/workspace-dialogs.css` | the workspace's dialogs, prefix `workspace-dialog-` |
| 8 | `app/search-workspace/panels/visualization-panel.css` | the column and bar charts, prefix `visualization-` |

The feature files come last on purpose. A feature rule and a shared rule
regularly have the same specificity — `.reports-table-wrap` against
`.table-wrap`, both one class — and the feature is the one meant to win. Until
Phase 4 those five files were CSS modules, which the bundler happened to emit
after `globals.css`: the order was real and load-bearing and nothing wrote it
down. Adding a stylesheet is a line in `index.css`, in the position its rules
need.
...
**Buttons.** `app/globals.css`, the `/* Buttons */` banner. `.button` is the one
...
**Tables.** `app/globals.css`, the `/* Dense result grids */` banner.
...
**The product chrome.** `app/globals.css`, the `/* Multi-page product shell */`
banner, and `app/_components/product-shell.tsx`. Every route renders through
...
**One page.** Its own file, from the table above. If the page has no file yet,
its rules are in `app/globals.css` under a banner named after the page; a page
whose rules have grown past a screenful is a candidate for a colocated file and
a prefix, added to `index.css` in the feature block.
...
named CSS colour, anywhere in `app/globals.css`, a feature stylesheet, an inline
...
off-canon step rather than six different steps. The `max-height: 650px` query in
