...
const COLLECTORS = [
  { host: "api-prod-03", lastSeen: "1 sec ago", queueDepth: "0 events", throughput: "214 evt/s", destination: "gradethis", peakPercent: 72 },
  { host: "worker-prod-02", lastSeen: "3 sec ago", queueDepth: "42 events", throughput: "87 evt/s", destination: "platform", peakPercent: 36 },
] as const;

const NAV_ITEMS: Array<{ key: AdminSection; label: string; detail: string; icon: AppIconName }> = [
  { key: "overview", label: "System overview", detail: "Health and capacity", icon: "dashboard" },
  { key: "indexes", label: "Indexes", detail: "Storage and retention", icon: "database" },
  { key: "collectors", label: "Data inputs", detail: "Collectors and tokens", icon: "download" },
  { key: "access", label: "Users & access", detail: "Roles and authentication", icon: "users" },
  { key: "server", label: "Server settings", detail: "Limits and preferences", icon: "settings" },
];
...
function DemoAdminConsole() {
  ...
  const primaryAction = section === "indexes"
    ? <button className="button button--primary" type="button" onClick={openIndexDialog}><AppIcon name="plus" size="sm" /> Simulate index</button>
    : section === "collectors"
      ? <button className="button button--primary" type="button" onClick={() => setModal("token")}>Generate demo token</button>
      : undefined;
    ...
    <div className="suite-page admin-page">
        ...
        <aside className="admin-sidebar" aria-label="Administration navigation">
          ...
          {NAV_ITEMS.map((item) => (
            <button className={section === item.key ? "active" : undefined} type="button" aria-current={section === item.key ? "page" : undefined} onClick={() => navigateSection(item.key)} key={item.key}>
              <i aria-hidden="true"><AppIcon name={item.icon} size="md" /></i><span><strong>{item.label}</strong><small>{item.detail}</small></span><b aria-hidden="true"><AppIcon name="chevron-right" size="xs" /></b>
            </button>
          ))}
          <div className="admin-sidebar-meta"><StatusDot tone="success" /><div><strong>open-splunk.local</strong><small>{OPEN_SPLUNK_BUILD_LABEL} · Preview data</small></div></div>
        </aside>
      ...
      {modal === "index" ? (
        <Modal
          ...
          footer={<><button className="button button--secondary" type="button" onClick={() => setModal(null)}>Cancel</button><button className="button button--primary" type="submit" form="create-index-form" disabled={indexName.trim().length === 0}>Add preview index</button></>}
        >
      ...
      {modal === "token" ? (
        <Modal
          ...
          footer={tokenSecret === null ? <><button className="button button--secondary" type="button" onClick={closeTokenDialog}>Cancel</button><button className="button button--primary" type="submit" form="create-token-form" disabled={tokenName.trim().length === 0}>Generate demo value</button></> : <button className="button button--primary" type="button" onClick={closeTokenDialog}>Close</button>}
        >
          {tokenSecret === null ? (
      ...
      {toast === null ? null : <output className="toast toast-success"><span><AppIcon name="check" size="sm" /></span><strong>{toast}</strong><button type="button" aria-label="Dismiss notification" onClick={() => setToast(null)}><AppIcon name="close" size="md" /></button></output>}
    </div>
  );
}
...
function OverviewSection({ onNavigate }: { onNavigate: (section: AdminSection) => void }) {
  return (
    <div className="admin-section-stack">
      <header className="admin-section-header"><div><h2>System overview</h2><p>Current health, storage, and administrative attention.</p></div><StatusLabel tone="success">Operational</StatusLabel></header>
      ...
      <div className="admin-overview-grid">
        <section className="suite-card"><header className="suite-card-header"><div><h3>Service health</h3><p>Core components and current latency.</p></div><button type="button">Refresh</button></header><ul className="health-service-list"><li><StatusDot tone="success" /><div><strong>Search API</strong><small>SRouter protobuf · p95 118 ms</small></div><b>Healthy</b></li><li><StatusDot tone="success" /><div><strong>ClickHouse</strong><small>1 node · 11 ms query ping</small></div><b>Healthy</b></li><li><StatusDot tone="success" /><div><strong>Control plane</strong><small>SQLite WAL · last checkpoint 2 min ago</small></div><b>Healthy</b></li><li><StatusDot tone="warning" /><div><strong>Export worker</strong><small>1 artifact awaiting cleanup</small></div><b className="warning-copy">Attention</b></li></ul></section>
        <section className="suite-card"><header className="suite-card-header"><div><h3>Administrative activity</h3><p>Recent configuration changes.</p></div><Link href="/activity/jobs/">Audit log</Link></header><ol className="admin-activity-list"><li><span>SU</span><div><strong>Search limits updated</strong><small>Administrator · 18 minutes ago</small></div></li><li><span>TK</span><div><strong>Ingestion token used</strong><small>prod-gradethis · 31 minutes ago</small></div></li><li><span>IX</span><div><strong>Index retention confirmed</strong><small>gradethis · Yesterday</small></div></li></ol></section>
      </div>
    </div>
  );
}
...
function IndexesSection({ filter, indexes, onFilterChange, onToggle }: { filter: string; indexes: IndexRow[]; onFilterChange: (value: string) => void; onToggle: (name: string) => void }) {
  ...
  return (
    <div className="admin-section-stack">
      ...
      <div className="resource-toolbar"><label><span className="sr-only">Filter indexes</span><i aria-hidden="true"><AppIcon name="search" size="sm" /></i><input value={filter} onChange={(event) => onFilterChange(event.target.value)} placeholder="Filter indexes" /></label><button className="button button--toolbar" type="button" disabled aria-describedby="demo-index-toolbar-note">All statuses unavailable</button><button className="button button--toolbar" type="button" disabled aria-describedby="demo-index-toolbar-note">Columns unavailable</button></div>
      {indexes.length === 0 ? (
        <div className="product-empty-state"><span aria-hidden="true"><AppIcon name="search" size="lg" /></span><strong>No matching indexes</strong><p>Try another index name.</p><button type="button" onClick={() => onFilterChange("")}>Clear filter</button></div>
      ) : (
        <div className="suite-card resource-table-card"><div className="table-wrap"><table className="table admin-resource-table"><caption className="sr-only">Preview indexes</caption><thead><tr><th scope="col">Name</th><th scope="col">Status</th><th scope="col">Events today</th><th scope="col">Storage</th><th scope="col">Retention</th><th scope="col">Last event</th><th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody>{indexes.map((item) => <tr key={item.name}><td><Link className="resource-name" href={searchLaunchHref(boundedIndexSearchQuery(item.name))} aria-label={`Search index ${item.name}`}><span aria-hidden="true"><AppIcon name="database" size="md" /></span><div><strong>{item.name}</strong><small>index={item.name}</small></div></Link></td><td><StatusLabel tone={item.status === "Active" ? "success" : "neutral"}>{item.status}</StatusLabel></td><td className="numeric-data">{item.events}</td><td>{item.storage}</td><td>{item.retention}</td><td>{item.lastEvent}</td><td><div className="resource-action-wrap" ref={openActions === item.name ? openActionWrapRef : undefined}><button ref={(element) => { if (element === null) actionTriggerRefs.current.delete(item.name); else actionTriggerRefs.current.set(item.name, element); }} className="row-overflow" type="button" aria-label={`Actions for ${item.name}`} aria-haspopup="menu" aria-expanded={openActions === item.name} onClick={(event) => toggleActions(item.name, event.currentTarget)}><AppIcon name="more" size="md" /></button>{openActions === item.name ? <div className="resource-action-menu" role="menu" aria-label={`Actions for ${item.name}`}><Link role="menuitem" href={searchLaunchHref(boundedIndexSearchQuery(item.name))}>Search this index</Link><button role="menuitem" type="button" onClick={() => runToggleAction(item.name)}>{item.status === "Active" ? "Simulate pausing ingestion" : "Simulate resuming ingestion"}</button></div> : null}</div></td></tr>)}</tbody></table></div></div>
      )}
...
function CollectorsSection() {
  return (
    <div className="admin-section-stack">
      <header className="admin-section-header"><div><h2>Data inputs</h2><p>Preview collector health, queue depth, and token workflows.</p></div><StatusLabel tone="success">2 connected</StatusLabel></header>
      <div className="collector-grid">{COLLECTORS.map((collector) => <article className="collector-card" key={collector.host}><header><span className="collector-host-icon">▣</span><div><strong>{collector.host}</strong><small>Linux · collector {OPEN_SPLUNK_BUILD_LABEL}</small></div><StatusLabel tone="success">Live</StatusLabel></header><dl><div><dt>Last seen</dt><dd>{collector.lastSeen}</dd></div><div><dt>Queue depth</dt><dd>{collector.queueDepth}</dd></div><div><dt>Throughput</dt><dd>{collector.throughput}</dd></div><div><dt>Destination</dt><dd>{collector.destination}</dd></div></dl><footer><span><i style={{ width: `${collector.peakPercent}%` }} /></span><small>{collector.peakPercent}% of configured peak</small><button className="button button--link" type="button">Details</button></footer></article>)}</div>
      <section className="suite-card token-section"><header className="suite-card-header"><div><h3>Issued credentials</h3><p>Sample records illustrating index-scoped ingestion tokens.</p></div></header><div className="table-wrap"><table className="table"><caption className="sr-only">Preview ingestion tokens</caption><thead><tr><th scope="col">Name</th><th scope="col">Prefix</th><th scope="col">Allowed indexes</th><th scope="col">Last used</th><th scope="col">Status</th><th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody><tr><td><strong>prod-gradethis</strong></td><td><code>ospl_ing_a73…</code></td><td>gradethis</td><td>1 sec ago</td><td><StatusLabel tone="success">Active</StatusLabel></td><td><button className="row-overflow" type="button" aria-label="Actions for prod-gradethis"><AppIcon name="more" size="md" /></button></td></tr><tr><td><strong>platform-workers</strong></td><td><code>ospl_ing_2f1…</code></td><td>platform</td><td>3 sec ago</td><td><StatusLabel tone="success">Active</StatusLabel></td><td><button className="row-overflow" type="button" aria-label="Actions for platform-workers"><AppIcon name="more" size="md" /></button></td></tr></tbody></table></div></section>
    </div>
  );
}
...
function AccessSection() {
  return (
    <div className="admin-section-stack"><header className="admin-section-header"><div><h2>Users &amp; access</h2><p>Preview accounts, roles, and index permissions.</p></div><button className="button button--primary" type="button" disabled aria-describedby="demo-authentication-unavailable">Invite user unavailable</button></header><div id="demo-authentication-unavailable" className="access-mode-notice"><span aria-hidden="true"><AppIcon name="info" size="md" /></span><div><strong>Authentication is not connected</strong><p>Administrator is a preview persona. The role model is illustrative and does not grant or enforce backend access.</p></div></div><section className="suite-card"><header className="suite-card-header"><div><h3>Administrators</h3><p>Sample users with full deployment access.</p></div><span>1 preview user</span></header><div className="user-row"><span className="user-avatar-large">A</span><div><strong>Administrator</strong><small>admin@localhost · Preview persona</small></div><span className="badge badge--info">admin</span><b>Fixture user</b><button aria-label="Actions for Administrator" className="row-overflow" type="button"><AppIcon name="more" size="md" /></button></div></section><section className="suite-card"><header className="suite-card-header"><div><h3>Role permissions</h3><p>Illustrative permissions for the preview administrator.</p></div></header><div className="permission-grid"><span><i aria-hidden="true"><AppIcon name="check" size="sm" /></i><strong>Search all indexes</strong><small>Read and export indexed events</small></span><span><i aria-hidden="true"><AppIcon name="check" size="sm" /></i><strong>Manage data</strong><small>Indexes, collectors, and tokens</small></span><span><i aria-hidden="true"><AppIcon name="check" size="sm" /></i><strong>Manage knowledge</strong><small>Saved searches and dashboards</small></span><span><i aria-hidden="true"><AppIcon name="check" size="sm" /></i><strong>Administer system</strong><small>Limits, users, and diagnostics</small></span></div></section></div>
  );
}
