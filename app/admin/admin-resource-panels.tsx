...
export function AppsAdminPanel({ apiBaseUrl, bootstrap }: PanelProps) {
  ...
  return (
    <div className="admin-section-stack">
      <header className="admin-section-header"><div><h2>Apps</h2><p>Manage UI and knowledge-object scopes, default indexes, and time ranges.</p></div><button className="button button--primary" type="button" onClick={() => { setForm(blankAppForm()); setTarget(null); setNotice(null); setModal("create"); }}><AppIcon name="plus" size="sm" /> Create app</button></header>
...
export function CollectorFleetPanel({ apiBaseUrl, bootstrap }: PanelProps) {
  ...
  return (
    <div className="admin-section-stack">
      <header className="admin-section-header"><div><h2>Collector fleet</h2><p>Inspect runtime health, durable queues, inputs, and administrative state.</p></div><button className="button" type="button" onClick={load}>Refresh</button></header>
      ...
      <div className="admin-pagination-footer"><strong>{countLabel(collectors.length, total, totalExact, "collector", "collectors")}</strong>{nextPageToken === null ? null : <button className="button button--secondary" type="button" disabled={loadingMore || busy} onClick={() => void loadMore()}>{loadingMore ? "Loading…" : "Load more collectors"}</button>}</div>

      {modal === "rename" && target !== null ? <Modal title={`Rename ${target.displayName || target.collectorId}`} subtitle="Set an operator-facing label or leave it blank to clear the label." onClose={() => !busy && setModal(null)} footer={<><button className="button button--secondary" type="button" disabled={busy} onClick={() => setModal(null)}>Cancel</button><button className="button button--primary" type="submit" form="rename-collector-form" disabled={busy}>{busy ? "Saving…" : "Save name"}</button></>}><form id="rename-collector-form" className="admin-form" onSubmit={(event) => void renameCollector(event)}>{notice === null ? null : <div className="access-mode-notice" role="alert"><span>!</span><div><strong>Collector could not be renamed</strong><p>{notice}</p></div></div>}<label htmlFor="collector-display-name"><span>Display name</span><input id="collector-display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder={target.collectorId} /></label></form></Modal> : null}
      {modal === "details" && target !== null ? <Modal wide title={target.displayName || target.collectorId} subtitle="Live collector metadata from the server." onClose={() => setModal(null)} footer={<button className="button button--primary" type="button" onClick={() => setModal(null)}>Close</button>}>
