        ...
        <div className="suite-card resource-table-card"><div className="responsive-table-wrap"><table className="product-table admin-resource-table">
          ...
          <tbody>{apps.map((app) => {
            ...
            return <tr key={app.appId}>
              <td><strong>{definition?.displayName || app.appId}</strong><small className="table-secondary">{definition?.slug ?? app.appId}{definition?.description ? ` · ${definition.description}` : ""}</small></td>
              <td>{definition?.defaultIndexNames.join(", ") || "Server default"}</td>
              ...
              <td><div className="row-actions"><button className="table-action" type="button" disabled={busy || definition === undefined} onClick={() => void openEditor(app)}>Edit</button><button className="table-action" type="button" disabled={busy || (app.state !== AppState.APP_STATE_ACTIVE && app.state !== AppState.APP_STATE_ARCHIVED)} onClick={() => void changeState(app)}>{app.state === AppState.APP_STATE_ACTIVE ? "Archive" : "Activate"}</button>{app.state === AppState.APP_STATE_ARCHIVED ? <button className="table-action" type="button" disabled={busy} onClick={() => { setTarget(app); setConfirmation(""); setNotice(null); setModal("delete"); }}>Delete</button> : null}</div></td>
            </tr>;
          })}</tbody>
        </table></div></div>
      )}
        ...
        <div className="suite-card resource-table-card"><div className="responsive-table-wrap"><table className="product-table admin-resource-table collector-fleet-table">
          ...
          <tbody>{collectors.map((collector) => {
            ...
            return <tr key={collector.collectorId}>
              ...
              <td>{collector.authorizedIndexes.join(", ") || "None"}</td>
