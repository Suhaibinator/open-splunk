...
// TestPipelineCommandsPreserveUntouchedSemanticBytesThroughManagerAgainstClickHouse
// composes every pipeline command while an unrelated semantic-BINARY _raw value is
...
func TestPipelineCommandsPreserveUntouchedSemanticBytesThroughManagerAgainstClickHouse(t *testing.T) {
	...
	for index, wantTag := range []string{"b", "a"} {
		...
		if !idOK || id != "sdet-v03-binary" || !tagOK || tag != wantTag ||
			...
			t.Fatalf("row %d public values did not preserve pipeline-command order and Bytes identity: %#v", index, row)
		}
	}
}
