...
// TestAuthoredExpressionExecutorManagerAgainstClickHouse is the transport-level
// authored expression acceptance gate. Trusted typed events pass through the production Store;
...
func TestAuthoredExpressionExecutorManagerAgainstClickHouse(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		if closeErr := container.Close(cleanupCtx); closeErr != nil {
			t.Errorf("close ClickHouse authored expression container: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := sequencer.Close(); closeErr != nil {
			t.Errorf("close authored expression visibility sequencer: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := store.Close(); closeErr != nil {
			t.Errorf("close authored expression store: %v", closeErr)
		}
	})
	...
	authoredExpressionStoreFixture(t, ctx, store, indexTime)
	...
	manager, err := searchjobs.New(searchjobs.Config{
		...
		NewID: func() string {
			return fmt.Sprintf("authored-expression-vertical-%02d", nextID.Add(1))
		},
	...
	compile := func(source string) clickhouse.CompiledQuery {
		t.Helper()
		logical := authoredExpressionVerticalPlan(t, indexTime, cutoff, source)
		compiled, compileErr := (clickhouse.Compiler{}).Compile(logical)
		if compileErr != nil {
			t.Fatalf("compile authored expression vertical SPL %q: %v", source, compileErr)
		}
		if !compiled.RequiresAtomicResult() {
			t.Fatalf("authored expression maximum shape did not require atomic execution: %q", source)
		}
		return compiled
	}
	...
	t.Run("typed arithmetic sparse null and signed zero through paging", func(t *testing.T) {
		...
		authoredExpressionAssertDouble(t, rows[0].Values[1], 6, "a sum")
		authoredExpressionAssertDouble(t, rows[0].Values[2], 6.25, "a numeric String")
		authoredExpressionAssertDouble(t, rows[0].Values[3], 4, "a Decimal")
		...
		authoredExpressionAssertDouble(t, rows[3].Values[1], 8, "d sum")
	})
	...
	t.Run("maximum arithmetic cancellation is prompt and publishes nothing", func(t *testing.T) {
		compiled := compile(authoredExpressionMaximumArithmeticSource())
		cancelContext, cancelQuery := context.WithCancel(ctx)
		defer cancelQuery()
		sink := &authoredExpressionCancelOnProgressSink{cancel: cancelQuery}
		started := time.Now()
	...
	t.Run("maximum membership obeys the native read limit atomically", func(t *testing.T) {
		compiled := compile(authoredExpressionMaximumMembershipSource())
	...
	t.Run("field catalog and summary preserve missing versus null", func(t *testing.T) {
		logical := authoredExpressionVerticalPlan(t, indexTime, cutoff,
	...
	t.Run("malformed semantic tag fails atomically with no visible prefix", func(t *testing.T) {
		authoredExpressionInsertMalformedDecimal(t, ctx, queryConnection, indexTime, cutoff)
		compiledPlan := authoredExpressionVerticalPlan(t, indexTime, cutoff,
...
func authoredExpressionStoreFixture(t *testing.T, ctx context.Context, store *clickhouse.Store, indexTime time.Time) {
	t.Helper()
	events := []*ingest.StoredEvent{
		authoredExpressionVerticalEvent(indexTime, "a-finite",
			authoredExpressionVerticalField("dyn_int", authoredExpressionVerticalSint(5)),
			authoredExpressionVerticalField("dyn_string", authoredExpressionVerticalString("12.5")),
			authoredExpressionVerticalField("dyn_decimal", authoredExpressionVerticalDecimal("3.25")),
			authoredExpressionVerticalField("status", authoredExpressionVerticalString("A")),
		),
		authoredExpressionVerticalEvent(indexTime, "b-explicit-null",
			authoredExpressionVerticalField("dyn_int", authoredExpressionVerticalNull()),
			authoredExpressionVerticalField("status", authoredExpressionVerticalString("B")),
		),
		authoredExpressionVerticalEvent(indexTime, "c-missing",
			authoredExpressionVerticalField("status", authoredExpressionVerticalNull()),
		),
		authoredExpressionVerticalEvent(indexTime, "d-finite",
			authoredExpressionVerticalField("dyn_int", authoredExpressionVerticalSint(7)),
			authoredExpressionVerticalField("status", authoredExpressionVerticalString("C")),
		),
	}
	digest := sha256.Sum256([]byte("authored-expression-vertical-fixture"))
...
func authoredExpressionVerticalEvent(
	indexTime time.Time,
	eventID string,
...
func authoredExpressionVerticalPlan(
	...
	if err != nil {
		t.Fatalf("parse authored expression vertical SPL %q: %v", source, err)
	}
	...
	if err != nil {
		t.Fatalf("build authored expression vertical SPL %q: %v", source, err)
	}
	return logical
}

func authoredExpressionInsertMalformedDecimal(
	t *testing.T,
	ctx context.Context,
	...
	if err := batch.Append(
		...
		nil, uint8(opensplunk.EventTimeSource_EVENT_TIME_SOURCE_PARSED), "sdet-host", "sdet-v02", "open-splunk:sdet",
		nil, uint8(opensplunk.LogSeverity_LOG_SEVERITY_INFO), nil, nil, []byte("redacted poison fixture"),
		uint8(opensplunk.RawEncoding_RAW_ENCODING_UTF8), nil, nil, document, []string{"malformed"},
...
func authoredExpressionAssertDouble(t *testing.T, value searchjobs.Value, want float64, label string) {
	t.Helper()
	got, ok := value.Double()
	if !ok || got != want {
...
type authoredExpressionCancelOnProgressSink struct {
	fakeSink
...
func (sink *authoredExpressionCancelOnProgressSink) ReportProgress(
...
func (sink *authoredExpressionCancelOnProgressSink) progressSnapshot() (
	int,
	uint64,
	time.Time,
) {
