...
// TestGradeThisCompatibilityV0_1AgainstClickHouse is the exact executable
// acceptance contract for the product architecture plan's ten initial
// GradeThis searches. One synthetic NDJSON profile flows through the collector
...
func TestGradeThisCompatibilityV0_1AgainstClickHouse(t *testing.T) {
	...
	manager, err := searchjobs.New(searchjobs.Config{
		...
		NewID: func() string {
			return fmt.Sprintf("gradethis-v0-1-%02d", nextID.Add(1))
		},
		CursorKey:   []byte("0123456789abcdef0123456789abcdef"),
		CursorScope: "gradethis-corpus-v0.1",
	})
	if err != nil {
		t.Fatal(err)
	}
...
type gradeThisDecodedFixture struct {
	...
	byID    map[string]*opensplunkv1.LogEvent
}
...
func gradeThisExpectations(t *testing.T, fixture gradeThisDecodedFixture) map[gradethiscorpus.SearchID]gradeThisExpectation {
	...
	event := func(id string) *opensplunkv1.LogEvent {
	...
	return map[gradethiscorpus.SearchID]gradeThisExpectation{
		...
		gradethiscorpus.SearchServerErrors: {
			schema: schema(
				column("_time", searchjobs.ValueKindTime),
				column("/api/v1/assessments", searchjobs.ValueKindUnsigned),
				column("/api/v1/submissions", searchjobs.ValueKindUnsigned),
			),
			rows: [][]searchjobs.Value{
		...
		gradethiscorpus.SearchResponses: {
			...
			rows: [][]searchjobs.Value{
				row(searchjobs.StringValue("/api/v1/assessments"), searchjobs.StringValue("200"), searchjobs.UnsignedValue(4)),
				row(searchjobs.StringValue("/api/v1/assessments"), searchjobs.StringValue("503"), searchjobs.UnsignedValue(3)),
				row(searchjobs.StringValue("/api/v1/submissions"), searchjobs.StringValue("200"), searchjobs.UnsignedValue(2)),
				row(searchjobs.StringValue("/api/v1/submissions"), searchjobs.StringValue("500"), searchjobs.UnsignedValue(1)),
			},
		},
		gradethiscorpus.SearchSlowRoutes: {
			...
			rows: [][]searchjobs.Value{
				row(searchjobs.StringValue("/api/v1/assessments"), searchjobs.UnsignedValue(7), searchjobs.DoubleValue(800)),
			},
		},
...
func gradeThisDynamicField(t *testing.T, event *opensplunkv1.LogEvent, name string) searchjobs.Value {
	t.Helper()
...
func gradeThisObjectValue(t *testing.T, object *opensplunkv1.TypedObject) searchjobs.Value {
	t.Helper()
	fields := slices.Clone(object.GetFields())
	slices.SortFunc(fields, func(left, right *opensplunkv1.TypedObjectField) int {
...
func gradeThisTypedValue(t *testing.T, value *opensplunkv1.TypedValue) searchjobs.Value {
	t.Helper()
	if value == nil {
