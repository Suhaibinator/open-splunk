...
type historyRecord struct {
	...
	CreatedAtUnixMicro  int64  `gorm:"column:created_at_unix_micro;type:integer;not null;index:search_history_created_idx,priority:1;index:search_history_owner_created_idx,priority:3,sort:desc;index:search_history_owner_app_created_idx,priority:4,sort:desc;index:search_history_owner_saved_created_idx,priority:4,sort:desc"`
...
// historyOwnerCountRecord is the GORM representation of the trigger-maintained
// search_history_owner_counts table. Application code reads it but never
// updates it directly.
type historyOwnerCountRecord struct {
	TenantID      string `gorm:"column:tenant_id;type:text;primaryKey;not null;check:search_history_owner_count_tenant_id_length,length(tenant_id) BETWEEN 1 AND 1024"`
	OwnerID       string `gorm:"column:owner_id;type:text;primaryKey;not null;check:search_history_owner_count_owner_id_length,length(owner_id) BETWEEN 1 AND 255"`
	TerminalCount int64  `gorm:"column:terminal_count;type:integer;not null;check:search_history_owner_count_positive,terminal_count > 0"`
}

func (historyOwnerCountRecord) TableName() string {
	return "search_history_owner_counts"
}
