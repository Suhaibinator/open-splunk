...
func TestNormalizeListRequestReturnsDetachedCanonicalPublicRequest(t *testing.T) {
	...
	want := ListRequest{
		...
		PageToken:          "opaque-token",
		IncludeTotal:       true,
		AppIDFilter:        new(testApp),
		OwnerIDFilter:      new(testOwner),
		TextFilter:         new("needle"),
		SelectorTextFilter: new("prod-*"),
		ObjectTypeFilters: []ObjectType{
			ObjectTypeFieldAlias,
			ObjectTypeFieldExtraction,
		},
...
func requestFromCanonicalWant(request ListRequest) ListRequest {
	return ListRequest{
		...
		AppIDFilter:         new(*request.AppIDFilter),
		OwnerIDFilter:       new(*request.OwnerIDFilter),
		TextFilter:          new(*request.TextFilter),
		...
		SelectorTextFilter:  new(*request.SelectorTextFilter),
