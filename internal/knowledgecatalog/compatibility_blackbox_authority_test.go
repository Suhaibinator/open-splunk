...
func TestCompatibilityV0_1CatalogBlackboxAuthorities(t *testing.T) {
	knowledgecompat.Run(t, knowledgecompat.OwnerKnowledgeCatalogBlackbox, map[knowledgecompat.Vector]knowledgecompat.Assertion{
		"lifecycle-state-machine": knowledgecompat.Exact([]knowledgecompat.WantCase{
			...
			t.Run("reenable-revalidation", compatibilityReenableLifecycle)
		}),
...
func compatibilityReenableLifecycle(t *testing.T) {
	...
	if object.GetVersion() != 1 || object.GetState() != opensplunkv1.KnowledgeObjectState_KNOWLEDGE_OBJECT_STATE_DRAFT {
		t.Fatalf("created lifecycle object = %v", object)
	}

	activate := func(version uint64, requestID string) *opensplunkv1.KnowledgeObject {
		t.Helper()
		response, err := harness.writer.SetState(harness.actorCtx, harness.writeScope, &opensplunkv1.SetKnowledgeObjectStateRequest{
	...
	disable := func(version uint64, requestID string) *opensplunkv1.KnowledgeObject {
		t.Helper()
		response, err := harness.writer.SetState(harness.actorCtx, harness.writeScope, &opensplunkv1.SetKnowledgeObjectStateRequest{
			KnowledgeObjectId: object.GetKnowledgeObjectId(),
			ExpectedVersion:   version,
	...
	if activeV2.GetVersion() != 2 || activeV2.GetState() != opensplunkv1.KnowledgeObjectState_KNOWLEDGE_OBJECT_STATE_ACTIVE {
		t.Fatalf("active v2 = %v", activeV2)
	}
	disabledV3 := disable(2, "compat-lifecycle-disable-0003")
	if disabledV3.GetVersion() != 3 || disabledV3.GetState() != opensplunkv1.KnowledgeObjectState_KNOWLEDGE_OBJECT_STATE_DISABLED {
		t.Fatalf("disabled v3 = %v", disabledV3)
	}

	editedDefinition := proto.Clone(disabledV3.GetDefinition()).(*opensplunkv1.KnowledgeObjectDefinition)
	description := "disabled candidate revalidated on activation"
	editedDefinition.Description = &description
	edited, err := harness.writer.Update(harness.actorCtx, harness.writeScope, &opensplunkv1.UpdateKnowledgeObjectRequest{
		KnowledgeObjectId: object.GetKnowledgeObjectId(),
		ExpectedVersion:   3,
	...
	if edited.GetKnowledgeObject().GetVersion() != 4 ||
		edited.GetKnowledgeObject().GetState() != opensplunkv1.KnowledgeObjectState_KNOWLEDGE_OBJECT_STATE_DISABLED {
		t.Fatalf("edited v4 = %v", edited.GetKnowledgeObject())
	}
	activeV5 := activate(4, "compat-lifecycle-reenable-0005")
	if activeV5.GetVersion() != 5 || activeV5.GetState() != opensplunkv1.KnowledgeObjectState_KNOWLEDGE_OBJECT_STATE_ACTIVE ||
		activeV5.GetDefinition().GetDescription() != description {
		t.Fatalf("reactivated v5 = %v", activeV5)
	}

	deleted, err := harness.writer.Delete(harness.actorCtx, harness.writeScope, &opensplunkv1.DeleteKnowledgeObjectRequest{
		KnowledgeObjectId: object.GetKnowledgeObjectId(),
		ExpectedVersion:   5,
