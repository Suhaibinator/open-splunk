...
// accepts CountRows, CountValues, or a nullable numeric Sum over one exact
// input field. IncludeCurrent chooses whether the frame ends at the current row
// or the immediately preceding row; WindowRows is zero for the complete
// admitted prefix. A positive grouped window is valid only with Global=false,
// which gives every exact BY tuple an independent row window.
