...
func TestBuildStrftimePreservesTypedIRAndSearchTimezone(t *testing.T) {
	t.Parallel()
	...
	logical, err := Build(
		mustParse(
			t,
			`index=gradethis | eval rendered=strftime(now(), "%Y-%m-%dT%H:%M:%S.%Q%:z")`,
		),
		scope,
	)
	if err != nil {
	...
	if call.Function != ScalarFunctionStrftime || len(call.Arguments) != 2 {
		t.Fatalf("expression = %#v", call)
	}
	if call.Arguments[0].(*ScalarCallExpression).Function != ScalarFunctionNow {
		t.Fatalf("time argument = %#v", call.Arguments[0])
	}
	format := call.Arguments[1].(*ScalarLiteralExpression)
	if format.Value.Kind != ValueKindString ||
		format.Value.String != "%Y-%m-%dT%H:%M:%S.%Q%:z" {
...
func TestBuildStrftimeRejectsForgedArityFormatBooleanEnumAndTypedNil(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name:       "zero arguments",
			expression: &spl.ScalarCallExpr{Function: spl.ScalarFunctionStrftime, Range: sourceRange},
			code:       "SPL_INVALID_EVAL_ARITY",
		},
		{
			name: "one argument",
			expression: &spl.ScalarCallExpr{
				Function:  spl.ScalarFunctionStrftime,
		...
		},
		{
			...
			expression: &spl.ScalarCallExpr{
				Function:  spl.ScalarFunctionStrftime,
				Arguments: []spl.ScalarExpr{typedNil, format("%F", true)},
				Range:     sourceRange,
			},
		...
		},
		{
			...
			expression: &spl.ScalarCallExpr{
				Function:  spl.ScalarFunctionStrftime,
				Arguments: []spl.ScalarExpr{value(), typedNilFormat},
				Range:     sourceRange,
			},
			code: "SPL_UNSUPPORTED_EVAL_EXPRESSION",
		},
		{
			...
			expression: &spl.ScalarCallExpr{
				Function:  spl.ScalarFunctionStrftime,
				Arguments: []spl.ScalarExpr{value(), format("%F", false)},
				Range:     sourceRange,
			},
			code: "SPL_UNSUPPORTED_EVAL_EXPRESSION",
		},
		...
		},
		{
			...
			expression: &spl.ScalarCallExpr{
				Function:  spl.ScalarFunctionStrftime,
				Arguments: []spl.ScalarExpr{value(), format("%c", true)},
				Range:     sourceRange,
			},
		...
		},
		{
			...
			expression: &spl.ScalarCallExpr{
				...
				Arguments: []spl.ScalarExpr{value(), format("%F", true)},
				Range:     sourceRange,
			},
