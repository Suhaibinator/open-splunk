...
func TestV03AnalysisAccountsForEveryReadWithoutInventingPrivateDependencies(t *testing.T) {
	t.Parallel()
	...
	scope.SearchJobID = "v03-analysis-job"
...
func TestV03FieldAnalysisRejectsOnlyTheRowGeneratingExpansionAtItsExactRange(t *testing.T) {
	t.Parallel()
	...
	scope := testScope([]string{"gradethis"}, nil)
	scope.SearchJobID = "v03-field-analysis-job"
	...
	if err := ValidateFieldAnalysisEligibility(eventLogical); err != nil {
		t.Fatalf("event-preserving v0.3 commands rejected for field analysis: %v", err)
	}
