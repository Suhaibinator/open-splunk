...
func TestBackupToIncludesCommittedWALAndExcludesUncommittedRows(t *testing.T) {
	...
	if identity.LatestVersion != 32 || identity.SHA256 == ([sha256.Size]byte{}) {
...
func TestVerifyCurrentMigrationsRequiresExactLedgerAndStableIdentity(t *testing.T) {
	...
	if firstIdentity != secondIdentity || firstIdentity.LatestVersion != 32 {
		t.Fatalf("migration identities differ: first=%+v second=%+v", firstIdentity, secondIdentity)
	}

	incomplete := open("incomplete")
	if _, err := incomplete.SQLDB().ExecContext(ctx, `DELETE FROM schema_migrations WHERE version = 32`); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := drifted.SQLDB().ExecContext(ctx, `UPDATE schema_migrations SET name = '0032_changed.sql' WHERE version = 32`); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := tooNew.SQLDB().ExecContext(ctx, `
		INSERT INTO schema_migrations (version, name, checksum, applied_at_unix_micro)
			VALUES (33, '0033_future.sql', zeroblob(32), 1)`); err != nil {
		t.Fatal(err)
	}
