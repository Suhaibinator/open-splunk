...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 30 {
		t.Fatalf("schema migration count = %d, want 30", migrationCount)
	}
	...
	if migrationCount != 30 {
		t.Fatalf("schema migration count after reopen = %d, want 30", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 30 {
		t.Fatalf("schema migration count = %d, want 30", count)
	}
}
