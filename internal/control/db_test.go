...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 38 {
		t.Fatalf("schema migration count = %d, want 38", migrationCount)
	}
	...
	if migrationCount != 38 {
		t.Fatalf("schema migration count after reopen = %d, want 38", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 38 {
		t.Fatalf("schema migration count = %d, want 38", count)
	}
}
...
func migrationsBefore(t *testing.T, cutoff string) fstest.MapFS {
	t.Helper()
	return testsupport.SQLiteMigrationsBefore(t, cutoff)
}
