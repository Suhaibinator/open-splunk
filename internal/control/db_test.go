...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 36 {
		t.Fatalf("schema migration count = %d, want 36", migrationCount)
	}
	...
	if migrationCount != 36 {
		t.Fatalf("schema migration count after reopen = %d, want 36", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 36 {
		t.Fatalf("schema migration count = %d, want 36", count)
	}
}
