...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 23 {
		t.Fatalf("schema migration count = %d, want 23", migrationCount)
	}
	...
	if migrationCount != 23 {
		t.Fatalf("schema migration count after reopen = %d, want 23", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 23 {
		t.Fatalf("schema migration count = %d, want 23", count)
	}
}
