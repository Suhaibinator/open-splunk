...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 32 {
		t.Fatalf("schema migration count = %d, want 32", migrationCount)
	}
	...
	if migrationCount != 32 {
		t.Fatalf("schema migration count after reopen = %d, want 32", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 32 {
		t.Fatalf("schema migration count = %d, want 32", count)
	}
}
