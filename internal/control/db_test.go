...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 39 {
		t.Fatalf("schema migration count = %d, want 39", migrationCount)
	}
	...
	if migrationCount != 39 {
		t.Fatalf("schema migration count after reopen = %d, want 39", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 39 {
		t.Fatalf("schema migration count = %d, want 39", count)
	}
}
