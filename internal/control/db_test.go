...
func TestOpenConfiguresSQLiteAndAppliesMigrations(t *testing.T) {
	...
	if migrationCount != 31 {
		t.Fatalf("schema migration count = %d, want 31", migrationCount)
	}
	...
	if migrationCount != 31 {
		t.Fatalf("schema migration count after reopen = %d, want 31", migrationCount)
	}
}
...
func TestConcurrentOpenSerializesMigrationStartup(t *testing.T) {
	...
	if count != 31 {
		t.Fatalf("schema migration count = %d, want 31", count)
	}
}
