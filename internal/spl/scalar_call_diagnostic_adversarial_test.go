...
func TestParseScalarCallArityBoundariesKeepInvalidEvalArityMessages(t *testing.T) {
	...
	for _, test := range []struct{ call, message string }{
		...
		{`tonumber()`, "tonumber requires exactly one argument"},
		{`tonumber(a, b)`, "tonumber requires exactly one argument"},
		{`tostring()`, "tostring requires one or two arguments"},
		{`tostring(a, "hex", 1)`, "tostring requires one or two arguments"},
		...
		{`split()`, "split requires exactly two arguments"},
		{`split(a)`, "split requires exactly two arguments"},
		{`split(a, ",", b)`, "split requires exactly two arguments"},
		{`mvappend()`, "mvappend requires at least one argument"},
		{`mvdedup()`, "mvdedup requires exactly one argument"},
		{`mvdedup(a, b)`, "mvdedup requires exactly one argument"},
		{`mvindex()`, "mvindex requires two or three arguments"},
		{`mvindex(a)`, "mvindex requires two or three arguments"},
		{`mvindex(a, 0, 1, 2)`, "mvindex requires two or three arguments"},
		{`mvjoin()`, "mvjoin requires exactly two arguments"},
		{`mvjoin(a)`, "mvjoin requires exactly two arguments"},
		{`mvjoin(a, ",", b)`, "mvjoin requires exactly two arguments"},
		{`mvzip()`, "mvzip requires two or three arguments"},
		{`mvzip(a)`, "mvzip requires two or three arguments"},
		{`mvzip(a, b, ",", c)`, "mvzip requires two or three arguments"},
		{`mvfind()`, "mvfind requires exactly two arguments"},
		{`mvfind(a)`, "mvfind requires exactly two arguments"},
		{`mvfind(a, "b", "c")`, "mvfind requires exactly two arguments"},
