...
func FuzzParseAuthoredExpressionsAreDeterministicAndAtomic(f *testing.F) {
	seeds := []string{
	...
	f.Fuzz(func(t *testing.T, source string) {
		authoredFuzzCheckParse(t, source, true)
	})
}

func FuzzParseAuthoredExpressionFragmentsCanonicalize(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, expression string, predicate bool) {
		...
		authoredFuzzCheckParse(t, prefix+expression, true)
	})
}

func authoredFuzzCheckParse(t *testing.T, source string, canonicalize bool) {
	t.Helper()
	...
	if firstErr != nil {
		...
		firstDiagnostic := authoredFuzzDiagnostic(t, firstErr)
		secondDiagnostic := authoredFuzzDiagnostic(t, secondErr)
		...
		authoredFuzzValidateRange(t, source, firstDiagnostic.Range, "diagnostic")
		return
	}
	if first == nil || second == nil {
	...
	authoredFuzzValidateNodeRanges(t, source, first)
	if !canonicalize {
		return
	}

	canonical, ok := authoredFuzzCanonicalQuery(first)
	...
	authoredFuzzValidateNodeRanges(t, canonical, reparsed)
	secondCanonical, ok := authoredFuzzCanonicalQuery(reparsed)
	if !ok || secondCanonical != canonical {
		t.Fatalf("canonical form is not idempotent: first=%q second=%q", canonical, secondCanonical)
	}
}

func authoredFuzzDiagnostic(t *testing.T, err error) *Diagnostic {
	t.Helper()
...
func authoredFuzzValidateNodeRanges(t *testing.T, source string, root Node) {
	t.Helper()
	...
	walk = func(value reflect.Value) {
		...
		if value.Kind() == reflect.Pointer {
			...
			if value.CanInterface() && value.Type().Implements(nodeType) {
				...
				authoredFuzzValidateRange(t, source, node.SourceRange(), value.Type().String())
			}
			walk(value.Elem())
			return
		}
		if value.Type() == rangeType && value.CanInterface() {
			...
			if sourceRange != (Range{}) {
				authoredFuzzValidateRange(t, source, sourceRange, "AST field")
			}
			return
		}
		switch value.Kind() {
...
func authoredFuzzValidateRange(t *testing.T, source string, sourceRange Range, label string) {
	t.Helper()
...
func authoredFuzzCanonicalQuery(query *Query) (string, bool) {
	...
	case *EvalCommand:
		...
		destination, ok := authoredFuzzCanonicalField(command.Assignments[0].Field)
		if !ok {
			return "", false
		}
		expression, ok := authoredFuzzCanonicalScalar(command.Assignments[0].Expression)
		if !ok {
			return "", false
		}
	...
	case *WhereCommand:
		...
		expression, ok := authoredFuzzCanonicalWhere(command.Expression)
		if !ok {
			return "", false
		}
...
func authoredFuzzCanonicalScalar(expression ScalarExpr) (string, bool) {
	switch expression := expression.(type) {
	case *ScalarFieldExpr:
		if expression == nil {
			return "", false
		}
		return authoredFuzzCanonicalField(expression.Field)
	case *ScalarLiteralExpr:
		if expression == nil {
			return "", false
		}
		if expression.Value.Kind == LiteralKindString {
			return authoredFuzzCanonicalString(expression.Value.Text)
		}
	...
	case *ScalarUnaryExpr:
		...
		operand, ok := authoredFuzzCanonicalScalar(expression.Operand)
		if !ok {
			return "", false
		}
		operator := ""
	...
	case *ScalarBinaryExpr:
		...
		left, leftOK := authoredFuzzCanonicalScalar(expression.Left)
		right, rightOK := authoredFuzzCanonicalScalar(expression.Right)
		if !leftOK || !rightOK {
			return "", false
		}
		operator := ""
	...
	case *ScalarCallExpr:
		...
		for index, argument := range expression.Arguments {
			canonical, ok := authoredFuzzCanonicalScalar(argument)
			if !ok {
				return "", false
			}
		...
		name, ok := authoredFuzzCanonicalFunctionName(expression.Function)
		if !ok {
			return "", false
		}
	...
	case *ScalarIfExpr:
		...
		condition, conditionOK := authoredFuzzCanonicalWhere(expression.Condition)
		trueValue, trueOK := authoredFuzzCanonicalScalar(expression.True)
		falseValue, falseOK := authoredFuzzCanonicalScalar(expression.False)
	...
	case *ScalarCaseExpr:
		...
		for _, branch := range expression.Branches {
			condition, conditionOK := authoredFuzzCanonicalWhere(branch.Condition)
			value, valueOK := authoredFuzzCanonicalScalar(branch.Value)
...
func authoredFuzzCanonicalWhere(expression WhereExpr) (string, bool) {
	switch expression := expression.(type) {
	case *WhereBoolExpr:
		if expression == nil {
			return "", false
		}
		left, leftOK := authoredFuzzCanonicalWhere(expression.Left)
		right, rightOK := authoredFuzzCanonicalWhere(expression.Right)
	...
	case *WhereNotExpr:
		...
		operand, ok := authoredFuzzCanonicalWhere(expression.Operand)
		if !ok {
			return "", false
		}
	...
	case *WhereMembershipExpr:
		...
		value, ok := authoredFuzzCanonicalScalar(expression.Value)
		if !ok {
			return "", false
		}
		candidates := make([]string, len(expression.Candidates))
		for index, candidate := range expression.Candidates {
			canonical, candidateOK := authoredFuzzCanonicalScalar(candidate)
			if !candidateOK {
				return "", false
			}
	...
	case *WhereScalarPredicateExpr:
		...
		return authoredFuzzCanonicalScalar(expression.Value)
	default:
		return "", false
	}
}

func authoredFuzzCanonicalField(field string) (string, bool) {
...
func authoredFuzzCanonicalString(value string) (string, bool) {
...
func authoredFuzzCanonicalFunctionName(function ScalarFunction) (string, bool) {
	switch function {
...
var benchmarkParseAuthoredQuery *Query

func BenchmarkParseAuthoredArithmeticOperators(b *testing.B) {
	for _, operators := range []int{1, 32, 128, MaximumArithmeticOperatorsPerQuery} {
		b.Run(fmt.Sprintf("operators_%03d", operators), func(b *testing.B) {
			...
			for range b.N {
				...
				benchmarkParseAuthoredQuery = query
			}
		})
	}
}

func BenchmarkParseAuthoredMembershipCandidates(b *testing.B) {
