...
func TestParseV02ArithmeticPrecedenceAssociativityAndGrouping(t *testing.T) {
	t.Parallel()
...
func TestParseV02WhereDisambiguatesScalarAndBooleanGroups(t *testing.T) {
	t.Parallel()
	...
	assertV02DiagnosticCode(t, `| eval value=(status=500 OR status=503)`, "SPL_UNSUPPORTED_EVAL_EXPRESSION")
...
func TestParseV02QuotedScalarFieldsAndDestinations(t *testing.T) {
	t.Parallel()
	...
	assertV02DiagnosticCode(t, `| eval value='bad\q'`, "SPL_INVALID_FIELD_QUOTE_ESCAPE")
	assertV02DiagnosticCode(t, string(append([]byte(`| eval value='bad`), []byte{0xff, '\''}...)), "SPL_INVALID_FIELD")
	assertV02DiagnosticCode(t, `| eval value='unterminated`, "SPL_UNTERMINATED_FIELD_QUOTE")
	assertV02DiagnosticCode(t, `| eval value=1+'unterminated`, "SPL_UNTERMINATED_FIELD_QUOTE")
	...
	assertV02DiagnosticCode(t, `| where 0%'.'>0`, "SPL_INVALID_FIELD")
	...
	assertV02DiagnosticCode(t, `1+'`+largeQuotedField+`'`, "SPL_QUERY_TOO_COMPLEX")
}

func TestParseV02MembershipFormsAndCategoryBoundary(t *testing.T) {
	t.Parallel()
	...
	assertV02DiagnosticCode(t, `| where (status IN (200))=true`, "SPL_UNSUPPORTED_MEMBERSHIP_SYNTAX")
	assertV02DiagnosticCode(t, `| eval selected=status IN (200,201)`, "SPL_UNSUPPORTED_EVAL_EXPRESSION")
	assertV02DiagnosticCode(t, `| eval selected=in(status,200)`, "SPL_UNSUPPORTED_EVAL_EXPRESSION")
...
func TestLexV02ScalarOperatorsAreContextSafeAndSourceExact(t *testing.T) {
	...
	for _, source := range []string{
		...
		assertV02DiagnosticCode(t, source, "SPL_UNSUPPORTED_ARITHMETIC_SYNTAX")
	}
	assertV02DiagnosticCode(t, `| where 0%.>0`, "SPL_UNSUPPORTED_ARITHMETIC_SYNTAX")
...
func TestLexV02MultibyteInvalidQuoteEscapeKeepsExactPositions(t *testing.T) {
	t.Parallel()
...
func TestParseV02ExpressionBudgetsAndClosedKnowledgeProfile(t *testing.T) {
	t.Parallel()
	...
	assertV02DiagnosticCode(t, chain(MaximumArithmeticOperatorsPerQuery+1), "SPL_QUERY_TOO_COMPLEX")
	...
	assertV02DiagnosticCode(t, unary(MaximumUnaryOperatorChain+1), "SPL_QUERY_TOO_COMPLEX")

	candidates := make([]string, MaximumMembershipCandidates+1)
	for index := range candidates {
		candidates[index] = "1"
	}
	assertV02DiagnosticCode(t, `| where value IN (`+strings.Join(candidates, `,`)+`)`, "SPL_QUERY_TOO_COMPLEX")
	...
	assertV02DiagnosticCode(t, maximumQuery+`| where value IN (1)`, "SPL_QUERY_TOO_COMPLEX")

	for _, source := range []string{`1+2`, `(field)`, `'field'`} {
		if _, err := ParseScalarExpression(source); err == nil {
			t.Fatalf("ParseScalarExpression(%q) accepted v0.2-only syntax", source)
		}
	}
...
func TestParseV02PreservesNonScalarTokenization(t *testing.T) {
	t.Parallel()
	...
	assertV02DiagnosticCode(t, `status IN (500,503)`, "SPL_UNSUPPORTED_EXPRESSION")
...
func TestAnalyzeScalarExpressionRejectsForgedV02Nodes(t *testing.T) {
	t.Parallel()
...
func assertV02DiagnosticCode(t *testing.T, source, code string) {
	t.Helper()
	_, err := Parse(source)
	if err == nil {
