...
func FuzzParseDoesNotPanic(f *testing.F) {
	for _, seed := range []string{
		...
		`index=main | streamstats current=f window=5 global=f sum(payload.bytes) AS prior_bytes BY host`,
		`index=main | streamstats current=f window=5 global=f avg(payload.bytes) AS prior_mean BY host`,
		`index=main | streamstats current=f window=5 global=f min(payload.bytes) AS prior_min BY host`,
		`index=main | streamstats sum(bytes*)`,
		`index=main | streamstats avg(bytes*)`,
		`index=main | streamstats min(bytes*)`,
		`index=main | streamstats sum(eval(bytes>0))`,
		`index=main | streamstats sum(bytes, duration`,
