...
func TestParseStreamStatsRejectsUnsupportedSurfaceAtSource(t *testing.T) {
	...
	tests := []struct {
		...
		{"other aggregate", `index=main | streamstats stdev(bytes)`, "SPL_UNSUPPORTED_STREAMSTATS_AGGREGATE", "stdev"},
...
func TestClassifyResultShapeTreatsStreamStatsAsRowPreserving(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{source: `index=main | streamstats max(bytes)`, want: ResultShape{Kind: ResultKindEvents}},
