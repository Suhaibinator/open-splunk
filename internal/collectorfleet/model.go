...
type fleetRecord struct {
	...
	DisplayName          *string             `gorm:"column:display_name;type:text;index:collector_fleet_tenant_display_id_idx,priority:2;check:collector_fleet_display_name_bounded,display_name IS NULL OR (length(CAST(display_name AS BLOB)) BETWEEN 1 AND 255 AND instr(display_name, char(0)) = 0)"`
...
type runtimeRecord struct {
	TenantID                      string  `gorm:"column:tenant_id;type:text;primaryKey;priority:1;not null;index:collector_runtime_tenant_hostname_id_idx,priority:1;index:collector_runtime_tenant_last_seen_id_idx,priority:1;index:collector_runtime_tenant_queued_bytes_id_idx,priority:1"`
	CollectorID                   string  `gorm:"column:collector_id;type:text;primaryKey;priority:2;not null;index:collector_runtime_tenant_hostname_id_idx,priority:3;index:collector_runtime_tenant_last_seen_id_idx,priority:3;index:collector_runtime_tenant_queued_bytes_id_idx,priority:3"`
	...
	LastSeenAtUnixMicro           int64   `gorm:"column:last_seen_at_unix_micro;type:integer;not null;index:collector_runtime_tenant_last_seen_id_idx,priority:2;check:collector_runtime_last_seen_valid,last_seen_at_unix_micro BETWEEN connected_at_unix_micro AND 253402300799999999"`
	...
	QueuedBytes                   int64   `gorm:"column:queued_bytes;type:integer;not null;index:collector_runtime_tenant_queued_bytes_id_idx,priority:2;check:collector_runtime_queued_bytes_nonnegative,queued_bytes >= 0"`
...
type authorizedIndexRecord struct {
	TenantID    string `gorm:"column:tenant_id;type:text;primaryKey;priority:1;not null;index:collector_authorized_indexes_tenant_index_name_collector_idx,priority:1"`
	CollectorID string `gorm:"column:collector_id;type:text;primaryKey;priority:2;not null;index:collector_authorized_indexes_tenant_index_name_collector_idx,priority:3"`
	IndexName   string `gorm:"column:index_name;type:text;primaryKey;priority:3;not null;index:collector_authorized_indexes_tenant_index_name_collector_idx,priority:2;check:collector_authorized_indexes_name_canonical,length(index_name) BETWEEN 1 AND 255 AND index_name = lower(index_name) AND index_name NOT GLOB '*[^a-z0-9_-]*' AND substr(index_name, 1, 1) GLOB '[a-z0-9]' AND instr(index_name, 'kvstore') = 0"`
}
...
func (collectorCatalogRevisionRecord) TableName() string {
	return "collector_catalog_revisions"
}
