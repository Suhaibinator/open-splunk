...
const (
	v03LiveVerticalTenant = "tenant-v03-live-vertical"
	v03LiveVerticalOwner  = "owner-v03-live-vertical"
	v03LiveVerticalIndex  = "v03-live-vertical"
)

type v03LiveVerticalSnapshotter uint64

func (snapshot v03LiveVerticalSnapshotter) VisibilityCutoff(context.Context) (uint64, error) {
	return uint64(snapshot), nil
}

func TestV03PinnedClickHousePublishesNullableListsThroughPagingAndExport(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		if closeErr := container.Close(cleanupContext); closeErr != nil {
			t.Errorf("close v0.3 live vertical ClickHouse: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := queryConnection.Close(); closeErr != nil {
			t.Errorf("close v0.3 live query connection: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := controlDB.Close(); closeErr != nil {
			t.Errorf("close v0.3 live control DB: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := sequencer.Close(); closeErr != nil {
			t.Errorf("close v0.3 live sequencer: %v", closeErr)
		}
	})
	...
	t.Cleanup(func() {
		if closeErr := store.Close(); closeErr != nil {
			t.Errorf("close v0.3 live store: %v", closeErr)
		}
	})

	eventAnchor := time.Date(2026, time.July, 21, 12, 0, 0, 0, time.UTC)
	indexTime := time.Date(2026, time.August, 12, 12, 0, 0, 0, time.UTC)
	binaryEvent := v03LiveVerticalEvent("event-04", eventAnchor.Add(4*time.Second), indexTime,
		v03LiveVerticalField("tags", v03LiveVerticalNull()),
		v03LiveVerticalField("n", v03LiveVerticalDouble(4)))
	...
	events := []*ingest.StoredEvent{
		v03LiveVerticalEvent("event-01", eventAnchor.Add(time.Second), indexTime,
			v03LiveVerticalField("tags", v03LiveVerticalString("a,b")),
			v03LiveVerticalField("n", v03LiveVerticalDouble(1))),
		v03LiveVerticalEvent("event-02", eventAnchor.Add(2*time.Second), indexTime,
			v03LiveVerticalField("tags", v03LiveVerticalString("")),
			v03LiveVerticalField("n", v03LiveVerticalDouble(2))),
		v03LiveVerticalEvent("event-03", eventAnchor.Add(3*time.Second), indexTime,
			v03LiveVerticalField("n", v03LiveVerticalDouble(3))),
		binaryEvent,
	}
	digest := sha256.Sum256([]byte("v03-live-nullable-list-vertical"))
	stored, err := store.Store(ctx, ingest.StoreBatch{
		TenantID:           v03LiveVerticalTenant,
		CollectorID:        "collector-v03-live",
		BatchID:            "batch-v03-live",
		BatchSequence:      1,
	...
	if err != nil || stored.Accepted != uint32(len(events)) {
		t.Fatalf("store v0.3 live fixture = (%+v, %v)", stored, err)
	}
	...
	searchManager, err := searchjobs.New(searchjobs.Config{
		Executor:        executor,
		Snapshotter:     v03LiveVerticalSnapshotter(cutoff),
		...
		NewID: func() string {
			return fmt.Sprintf("v03-live-vertical-%02d", nextJobID.Add(1))
		},
		CursorKey:   []byte("0123456789abcdef0123456789abcdef"),
		CursorScope: "v03-live-nullable-list-vertical",
	})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		if closeErr := searchManager.Close(); closeErr != nil {
			t.Errorf("close v0.3 live search manager: %v", closeErr)
		}
	})
	...
	access := searchjobs.AccessScope{TenantID: v03LiveVerticalTenant, OwnerID: v03LiveVerticalOwner}
	create := func(source string) searchjobs.Job {
		...
		job, createErr := searchManager.Create(ctx, searchjobs.CreateRequest{
			...
			AuthorizedIndexes: []string{v03LiveVerticalIndex},
			RequestedIndexes:  []string{v03LiveVerticalIndex},
			TimeRange:         resolvedRange,
		})
		if createErr != nil {
			t.Fatalf("create v0.3 live search: %v", createErr)
		}
		return v03WaitSearch(t, searchManager, access, job.ID)
	}

	unexpanded := create(`index=` + v03LiveVerticalIndex +
		` | sort 0 +event_id | makemv delim="," tags | table event_id tags`)
	unexpandedPages := v03CollectPages(t, searchManager, access, unexpanded.ID)
	...
	v03AssertNullableListRows(t, unexpandedRows)
	...
	exportManager, err := openexport.New(openexport.Config{
		...
		MaxWorkers:      1,
		CleanupInterval: -1,
		NewID:           func() string { return "v03-live-list-export" },
	})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		if closeErr := exportManager.Close(); closeErr != nil {
			t.Errorf("close v0.3 live export manager: %v", closeErr)
		}
	})
	...
	completedExport := v03WaitExport(t, exportManager, access, exportJob.ID)
	...
	v03AssertNullableListJSONLines(t, encoded)

	expanded := create(`index=` + v03LiveVerticalIndex +
		` | sort 0 +event_id | makemv delim="," tags | mvexpand tags limit=0 | table event_id tags`)
	expandedPages := v03CollectPages(t, searchManager, access, expanded.ID)
	...
	assertBinaryResult := func(source string, want []byte) {
		t.Helper()
		job := create(source)
		pages := v03CollectPages(t, searchManager, access, job.ID)
		if len(pages) != 1 || len(pages[0].Schema.Columns) != 1 ||
			...
			len(pages[0].Rows) != 1 {
			t.Fatalf("binary v0.3 result shape = %#v", pages)
		}
		got, ok := pages[0].Rows[0].Values[0].Bytes()
		if !ok || !slices.Equal(got, want) {
			t.Fatalf("binary v0.3 value = %#v / Bytes %t, want %v", pages[0].Rows[0].Values[0], ok, want)
		}
	}
	binaryRaw := []byte("binary-valid-utf8")
	assertBinaryResult(
		`index=`+v03LiveVerticalIndex+` event_id="event-04"`+
	...
	allTen := create(`index=` + v03LiveVerticalIndex +
	...
	allTenPages := v03CollectPages(t, searchManager, access, allTen.ID)
	if len(allTenPages) != 2 {
		t.Fatalf("all-ten result pages = %d, want 2", len(allTenPages))
	}
	wantAllTenFields := []string{
		"event_id", "tags", "running", "endpoint", "optional", "total", "step", "info_sid",
	}
	for pageIndex, page := range allTenPages {
		...
		for index, column := range page.Schema.Columns {
			...
			if strings.HasPrefix(strings.ToLower(column.Name), "__os_") {
				t.Fatalf("all-ten page %d leaked private column %q", pageIndex, column.Name)
			}
		}
		if !slices.Equal(gotFields, wantAllTenFields) {
			t.Fatalf("all-ten page %d fields = %v, want %v", pageIndex, gotFields, wantAllTenFields)
		}
	}
	allTenRows := append(slices.Clone(allTenPages[0].Rows), allTenPages[1].Rows...)
	wantAllTen := []struct {
		id, tag, endpoint, optional, sid string
		running, total, step             float64
		stepNull                         bool
	}{
		{id: "event-01", tag: "b", running: 10, endpoint: "v03-live-host/v03-live-source", optional: "unknown", total: 11, step: 1, sid: allTen.ID},
		{id: "event-01", tag: "a", running: 10, endpoint: "v03-live-host/v03-live-source", optional: "unknown", total: 11, step: 1, sid: allTen.ID},
		{id: "event-03", running: 7, endpoint: "v03-live-host/v03-live-source", optional: "unknown", total: 10, step: 3, sid: allTen.ID},
		{id: "event-04", running: 4, endpoint: "v03-live-host/v03-live-source", optional: "unknown", total: 8, stepNull: true, sid: allTen.ID},
	}
	if len(allTenRows) != len(wantAllTen) {
		t.Fatalf("all-ten rows = %d, want %d", len(allTenRows), len(wantAllTen))
	}
	for index, want := range wantAllTen {
		row := allTenRows[index]
		if row.Ordinal != uint64(index) {
			t.Fatalf("all-ten row %d ordinal = %d", index, row.Ordinal)
		}
		id, idOK := row.Values[0].String()
		tag := ""
		...
		if !tagNull {
			...
			if !tagOK {
				t.Fatalf("all-ten row %d tag = %#v", index, row.Values[1])
			}
		}
		...
		if !idOK || id != want.id || tag != want.tag || tagNull != (want.tag == "") ||
			...
			!sidOK || sid != want.sid {
			t.Fatalf("all-ten row %d = %#v, want %+v", index, row, want)
		}
	}
}

func v03LiveVerticalEvent(
	id string,
	eventTime time.Time,
	indexTime time.Time,
	...
	return &ingest.StoredEvent{
		...
		Event: &opensplunkv1.LogEvent{
			EventId:         id,
			IndexName:       v03LiveVerticalIndex,
...
func v03LiveVerticalField(name string, value *opensplunkv1.TypedValue) *opensplunkv1.TypedObjectField {
	return &opensplunkv1.TypedObjectField{Name: name, Value: value}
}

func v03LiveVerticalString(value string) *opensplunkv1.TypedValue {
	return &opensplunkv1.TypedValue{Kind: &opensplunkv1.TypedValue_StringValue{StringValue: value}}
}

func v03LiveVerticalDouble(value float64) *opensplunkv1.TypedValue {
	return &opensplunkv1.TypedValue{Kind: &opensplunkv1.TypedValue_DoubleValue{DoubleValue: value}}
}

func v03LiveVerticalNull() *opensplunkv1.TypedValue {
	return &opensplunkv1.TypedValue{
...
func v03AssertNullableListRows(t *testing.T, rows []searchjobs.ResultRow) {
	t.Helper()
	if len(rows) != 4 {
...
func v03AssertNullableListJSONLines(t *testing.T, encoded []byte) {
	t.Helper()
