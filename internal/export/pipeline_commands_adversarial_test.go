...
func TestPipelineExpandedPublicRowsExportWithoutContainerOrPrivateArtifacts(t *testing.T) {
	t.Parallel()

	const (
		searchJobID = "pipeline-export-search"
		source      = `index=main` +
	...
	access := searchjobs.AccessScope{TenantID: "tenant-pipeline-export", OwnerID: "owner-pipeline-export"}
	...
	searchManager, err := searchjobs.New(searchjobs.Config{
		Executor: integrationSearchExecutor(func(_ context.Context, query clickhouse.CompiledQuery, sink searchjobs.ResultSink) error {
			if !slices.Equal(query.OutputFields, []string{"event_id", "tags", "info_sid"}) ||
				...
				return fmt.Errorf("pipeline export compile = fields %v sealed %t atomic %t", query.OutputFields, query.HasValidExecutionSeal(), query.RequiresAtomicResult())
			}
			for _, field := range query.OutputFields {
				if strings.HasPrefix(strings.ToLower(field), "__os_") {
					return fmt.Errorf("pipeline export compile leaked private field %q", field)
				}
			}
	...
	if err != nil {
		t.Fatalf("create pipeline search: %v", err)
	}
	...
	exportManager, err := New(Config{
		...
		MaxWorkers:      1,
		CleanupInterval: -1,
		NewID:           func() string { return "pipeline-export-artifact" },
	})
	if err != nil {
	...
	if err != nil {
		t.Fatalf("create pipeline export: %v", err)
	}
	...
	if completed.Artifact == nil || completed.Artifact.RowCount != uint64(len(rows)) ||
		...
		t.Fatalf("completed pipeline export = %#v", completed)
	}
	contents, err := os.ReadFile(filepath.Join(exportManager.artifactDir, completed.Artifact.FileName))
	if err != nil {
		t.Fatalf("read pipeline artifact: %v", err)
	}
	want := "{\"tags\":\"[literal]\",\"event_id\":\"event-2\",\"info_sid\":\"pipeline-export-search\"}\n" +
		"{\"tags\":null,\"event_id\":\"event-1\",\"info_sid\":\"pipeline-export-search\"}\n" +
		"{\"tags\":\"\",\"event_id\":\"event-1\",\"info_sid\":\"pipeline-export-search\"}\n"
	if string(contents) != want {
		t.Fatalf("pipeline JSONL = %q, want %q", contents, want)
	}
