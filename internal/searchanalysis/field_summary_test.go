...
func TestFieldServiceSummaryRechecksScopeIdentityExpiryAndFingerprint(t *testing.T) {
	...
	stateMu.Lock()
	current.SearchTimezone = "America/Los_Angeles"
	stateMu.Unlock()
	if _, err := service.GetFieldSummary(context.Background(), access, request); err != nil {
		t.Fatalf("GetFieldSummary(changed search timezone) error = %v", err)
	}
	if compiler.Calls() != 4 || executor.Calls() != 4 {
		t.Fatalf("changed search timezone reused summary: compiler/executor %d/%d", compiler.Calls(), executor.Calls())
	}
