...
func TestV03ValidateCompilesAllTenWithoutExecutionOrRetainedState(t *testing.T) {
	var (
	...
	manager := newTestManager(t, Config{
		...
		NewID: func() string {
			idCalls.Add(1)
			return "validation-must-not-create-v03-job"
		},
		Now: func() time.Time {
			nowCalls.Add(1)
	...
	if err != nil || !result.Valid {
		t.Fatalf("Validate(all ten) = (%+v, %v), want valid", result, err)
	}
