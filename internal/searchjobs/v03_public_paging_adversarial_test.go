...
func TestV03AllTenCommandsRemainStableAndPrivateFreeAcrossPublicPaging(t *testing.T) {
	t.Parallel()

	const jobID = "v03-public-paging-1"
	...
	executor := executorFunc(func(_ context.Context, query clickhouse.CompiledQuery, sink ResultSink) error {
		if !reflect.DeepEqual(query.OutputFields, wantFields) {
			return fmt.Errorf("v0.3 public output fields = %v, want %v", query.OutputFields, wantFields)
		}
	...
	manager := newTestManager(t, Config{
		...
		MaxPageSize:     2,
		CleanupInterval: -1,
		NewID:           sequenceIDs("v03-public-paging"),
	})
	...
	if err != nil {
		t.Fatalf("Create v0.3 paging job: %v", err)
	}
	if created.ID != jobID {
	...
	for index, row := range gotRows {
		...
		if !v03PublicValuesEqual(row.Values, wantRows[index]) {
...
func TestV03AtomicExpansionFailurePublishesNoPublicPrefix(t *testing.T) {
	t.Parallel()
	...
	manager := newTestManager(t, Config{
		Executor:        executor,
		CleanupInterval: -1,
		NewID:           sequenceIDs("v03-atomic-expansion"),
	})
	request := validRequest()
...
func TestV03AtomicResultRowLimitStagesNoPreviewAndPublishesNothing(t *testing.T) {
	t.Parallel()
	...
	manager := newTestManager(t, Config{
		...
		MaxBytes:        1 << 20,
		CleanupInterval: -1,
		NewID:           sequenceIDs("v03-atomic-row-limit"),
	})
	request := validRequest()
...
func v03PublicValuesEqual(got, want []Value) bool {
	if len(got) != len(want) {
		return false
	}
...
func TestV03ManagerExecutesAllTenSPLWithSavedAndHistoryOriginMetadata(t *testing.T) {
	t.Parallel()
