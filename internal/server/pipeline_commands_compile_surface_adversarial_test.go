...
// pipelineCompilingSearchJobs is the real parser/planner/compiler seam behind the
...
// handling reaches a sealed pipeline-command physical program.
type pipelineCompilingSearchJobs struct {
	*fakeSearchJobs
	requests []searchjobs.CreateRequest
	queries  []clickhouse.CompiledQuery
}

func (jobs *pipelineCompilingSearchJobs) Create(
	_ context.Context,
	...
	logical, err := plan.Build(parsed, plan.Scope{
		...
		SearchJobID:       "job-pipeline-api-compile",
...
func requirePipelineAPICompiledCommands(t *testing.T, compiled clickhouse.CompiledQuery) {
	t.Helper()
	...
	if !compiled.HasValidExecutionSeal() || !compiled.RequiresAtomicResult() ||
		...
		t.Fatalf("API pipeline-command compile = sealed %t atomic %t fields %v", compiled.HasValidExecutionSeal(), compiled.RequiresAtomicResult(), compiled.OutputFields)
	}
	for _, field := range compiled.OutputFields {
		if strings.HasPrefix(strings.ToLower(field), "__os_") {
			t.Fatalf("API pipeline-command compile leaked private field %q", field)
		}
	}
}
