...
const pipelineFillNullMVExpandSource = "v03-fillnull-mvexpand-regression"

// TestCompilePipelineFillNullDynamicThenMVExpandMaterializesThePublicField protects
// the physical-column boundary between two independently valid pipeline stages.
...
func TestCompilePipelineFillNullDynamicThenMVExpandMaterializesThePublicField(t *testing.T) {
	t.Parallel()
...
func TestPipelineFillNullDynamicThenMVExpandAgainstClickHouse(t *testing.T) {
	...
	newEvent := func(id string, ordinal int, fields ...*opensplunk.TypedObjectField) *ingest.StoredEvent {
		event := testStoredEvent(id, "spl-v03-fillnull-mvexpand", indexTime)
		event.Event.Source = pipelineFillNullMVExpandSource
	...
	for _, barrier := range []string{
		...
		for rows.Next() {
			...
			got = append(got, id+"="+pipelineJSONText(value))
		}
		iterateErr := rows.Err()
		closeErr := rows.Close()
	...
	for _, barrier := range []string{
		...
		charted := compile(`index=spl-v03-fillnull-mvexpand source="` +
			pipelineFillNullMVExpandSource + `"` +
