...
func TestV03RegexCompilationAccountsRetainedAndEveryAuthoredProgram(t *testing.T) {
	t.Parallel()

	program := testKnowledgeEvidenceProgram(t, "v03-regex-accounting")
...
func TestV03RegexCompilerUsesExactPatternRangeForForgedResourceFailure(t *testing.T) {
	t.Parallel()
...
func TestV03ReverseAndAccumRetainPrivateCompleteOrder(t *testing.T) {
	t.Parallel()
...
func TestV03StrcatAllRequiredPreservesPriorDestination(t *testing.T) {
	t.Parallel()
...
func TestV03StrcatAllRequiredPreservesOptionalMultivalueNullness(t *testing.T) {
	t.Parallel()
...
func TestV03StrcatAllRequiredPreservesFlattenedContainerAuthority(t *testing.T) {
	t.Parallel()
	...
	for _, authority := range []string{"parent", "parent."} {
		if !v03ArgumentContainsString(compiled.Args, authority) {
