...
// BenchmarkAuthoredExpressionCompile is the deterministic compiler-side scaling
...
//	  -bench '^BenchmarkAuthoredExpressionCompile$' -benchtime=10x -count=3 -benchmem
func BenchmarkAuthoredExpressionCompile(b *testing.B) {
	b.ReportAllocs()
	for _, family := range []struct {
	...
	}{
		{
			...
			source: func(operators int) string {
				return authoredExpressionArithmeticBenchmarkSource("severity", operators)
			},
		},
		{
			...
			counts: []int{1, 32, 128, 256},
			source: func(operators int) string {
				return authoredExpressionArithmeticBenchmarkSource("duration_ms", operators)
			},
		},
		{
			name:   "membership",
			counts: []int{1, 8, 16, 32},
			source: authoredExpressionMembershipBenchmarkSource,
		},
	} {
...
func authoredExpressionArithmeticBenchmarkSource(field string, operators int) string {
...
func authoredExpressionMembershipBenchmarkSource(candidates int) string {
