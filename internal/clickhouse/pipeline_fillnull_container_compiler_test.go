...
func TestCompileFillNullPreservesFlattenedContainerTransport(t *testing.T) {
	...
	if strings.Contains(compiled.SQL, "fallback-界") ||
		!pipelineArgumentContainsString(compiled.Args, "fallback-界") ||
		!pipelineArgumentContainsString(compiled.Args, "parent.") {
...
func TestCompileFillNullDescendantAndOverlappingOrdersKeepIndependentSidecars(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !pipelineArgumentContainsString(compiled.Args, "parent.child") ||
				!pipelineArgumentContainsString(compiled.Args, "parent.child.") {
				t.Fatalf("descendant path authority is absent: %#v", compiled.Args)
			}
			if test.wantBindings == 2 && !pipelineArgumentContainsString(compiled.Args, "parent.") {
...
func TestPipelineFillNullPreservesFixedStringSemanticsBranchByBranch(t *testing.T) {
	t.Parallel()
...
func TestPipelineFillNullMaximumFixedStringShapeKeepsOneLayerPerField(t *testing.T) {
	t.Parallel()
	...
	for index := range fields {
		...
		fields[index] = pipelineCompilerField(t, name, sourceRange)
...
func TestPipelineFixedRawProvenancePipelinesCompileWithAtomicConsumers(t *testing.T) {
	t.Parallel()
...
func TestPipelineFillNullPreservesIndexCaseSensitiveComparison(t *testing.T) {
	t.Parallel()
