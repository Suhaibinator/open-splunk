...
// TestAuthoredExpressionAgainstClickHouse is the release's focused production-
...
func TestAuthoredExpressionAgainstClickHouse(t *testing.T) {
	...
	arithmeticEvent := authoredExpressionStoredEvent(
		"arithmetic",
		indexTime,
	...
	membershipMatch := authoredExpressionStoredEvent(
		"membership-match",
		indexTime,
		typedField("status", typedString("A")),
		typedField("numeric_status", typedSint(2)),
	)
	membershipCaseMismatch := authoredExpressionStoredEvent(
		"membership-case-mismatch",
		indexTime,
		typedField("status", typedString("a")),
		typedField("numeric_status", typedString("2")),
	)
	membershipNull := authoredExpressionStoredEvent(
		"membership-null",
		indexTime,
	...
	t.Run("fixed String arithmetic is rejected before execution", func(t *testing.T) {
		...
		logical, buildErr := plan.Build(parsed, authoredExpressionIntegrationScope(indexTime, 1))
		var rejection error
		if buildErr != nil {
			rejection = buildErr
		} else {
	...
	t.Run("maximum authored shapes execute with one scan", func(t *testing.T) {
		arithmeticSources := []struct {
		...
		}{
			{
				...
				source: strings.Replace(
					authoredExpressionArithmeticBenchmarkSource("severity", 256),
			...
			},
			{
				name: "Dynamic",
				source: strings.Replace(
					authoredExpressionArithmeticBenchmarkSource("dyn_int", 256),
					"index=gradethis", "index=expression-v02", 1,
				),
				wantPresent: 1,
			},
		...
		membershipSource := strings.ReplaceAll(
			strings.Replace(
				authoredExpressionMembershipBenchmarkSource(32),
				"index=gradethis", "index=expression-v02", 1,
			),
			"status", "dyn_int",
		)
...
func authoredExpressionStoredEvent(
	eventID string,
	indexTime time.Time,
...
func authoredExpressionIntegrationScope(indexTime time.Time, visibility uint64) plan.Scope {
	return plan.Scope{
		TenantID:          "tenant",
