...
// TestCompilePipelineFillNullBindsPrivatePhysicalColumns protects the
...
func TestCompilePipelineFillNullBindsPrivatePhysicalColumns(t *testing.T) {
	t.Parallel()
...
// TestPipelineFillNullAfterPrivatePhysicalProducersAgainstClickHouse covers real
...
// ClickHouse engine.
func TestPipelineFillNullAfterPrivatePhysicalProducersAgainstClickHouse(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			pipelineAssertJSONRows(t, queryContext, connection, compiled, test.fields, test.want)
		})
	}
	...
	t.Run("fixed Time stats BY direct", func(t *testing.T) {
		compiled := compile(base + ` event_id="one" | stats count BY _time` +
			` | fillnull value="fallback" _time | table _time count`)
		rows := pipelineJSONRows(t, queryContext, connection, compiled, []string{"_time", "count"})
		if len(rows) != 1 || len(rows[0]) != 2 || rows[0][0] == nil || pipelineJSONText(rows[0][1]) != "1" {
