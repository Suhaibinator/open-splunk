...
// TestV03AdversarialAgainstClickHouse exercises the complete v0.3 command
...
func TestV03AdversarialAgainstClickHouse(t *testing.T) {
	...
	stringsList := func(prefix string, count int) *opensplunkv1.TypedValue {
	...
	v03StoreIntegrationFixtureBatches(ctx, t, store, indexTime, resources)
	...
	v03StoreDeltaBoundaryFixtureBatches(ctx, t, store, indexTime, deltaBoundary)
	...
	if malformedVisibilityErr != nil {
		t.Fatalf("capture malformed v0.3 envelope visibility: %v", malformedVisibilityErr)
	}
	...
	if visibilityErr != nil {
		t.Fatalf("capture v0.3 visibility cutoff: %v", visibilityErr)
	}
	base := `index=spl-v03 source="v03-core"`

	t.Run("regex missing-null negation and Unicode RE2", func(t *testing.T) {
		v03AssertIDs(t, queryContext, connection, compile(
	...
	t.Run("reverse uses established private order through repeats head tail dedup aggregation and ties", func(t *testing.T) {
		...
		// The v0.2 tail contract itself returns its selected suffix in
		// reverse established order (06,05); v0.3 reverse must invert that
		...
		v03AssertJSONRows(t, queryContext, connection, compile(
	...
	t.Run("accum and delta share numeric eligibility and ordered windows", func(t *testing.T) {
		...
		v03AssertJSONRows(t, queryContext, connection, compiled,
		...
		v03AssertJSONRows(t, queryContext, connection, lagged,
		...
		v03AssertJSONRows(t, queryContext, connection, nonFiniteAccumInput,
		...
		v03AssertJSONRows(t, queryContext, connection, overflowAccumResult,
		...
		v03AssertJSONRows(t, queryContext, connection, nonFiniteDeltaInput,
		...
		v03AssertJSONRows(t, queryContext, connection, overflowDeltaResult,
	...
	t.Run("strcat missing policy fillnull type distinctions and row totals", func(t *testing.T) {
		...
		v03AssertJSONRows(t, queryContext, connection, allRequired,
		...
		v03AssertJSONRows(t, queryContext, connection, defaultRequired,
			[]string{"event_id", "joined"},
			[][]string{
		...
		v03AssertJSONRows(t, queryContext, connection, filled,
		...
		v03AssertJSONRows(t, queryContext, connection, totals,
		...
		nonFiniteTotals := compile(base + ` event_id="v03-01"` +
			...
			` | table finite_total`)
		v03AssertJSONRows(t, queryContext, connection, nonFiniteTotals,
		...
		v03AssertJSONRows(t, queryContext, connection, fillMultivalue,
		...
		v03AssertJSONRows(t, queryContext, connection, converted,
		...
		v03AssertJSONRows(t, queryContext, connection, filledMaximum,
		...
		v03AssertJSONRows(t, queryContext, connection, totaledMaximum,
	...
	t.Run("strcat allrequired preserves optional list nullness and flattened parents", func(t *testing.T) {
		optionalNull := compile(base + ` event_id="v03-05"` +
			...
			` | table tags_csv`)
		v03AssertJSONRows(t, queryContext, connection, optionalNull,
		...
		rows := v03JSONRows(t, queryContext, connection, container, []string{"event_id", "parent"})
		if len(rows) != 2 || len(rows[0]) != 2 || len(rows[1]) != 2 ||
			v03JSONText(rows[0][0]) != "v03-fillnull-container-missing" ||
			v03JSONText(rows[0][1]) != "<null>" ||
			v03JSONText(rows[1][0]) != "v03-fillnull-container-present" {
			t.Fatalf("strcat preserved container rows = %#v", rows)
		}
		parent, ok := v03DynamicStringMap(rows[1][1])
		if !ok || v03JSONText(parent["child"]) != "<null>" ||
			v03JSONText(parent["sibling"]) != "keep-界" {
	...
	t.Run("fillnull preserves flattened parents and exact descendant siblings", func(t *testing.T) {
		...
		parentRows := v03JSONRows(t, queryContext, connection, parent, []string{"event_id", "parent"})
		if len(parentRows) != 2 || len(parentRows[0]) != 2 || len(parentRows[1]) != 2 ||
			v03JSONText(parentRows[0][0]) != "v03-fillnull-container-missing" ||
			v03JSONText(parentRows[0][1]) != "fallback-界" ||
			v03JSONText(parentRows[1][0]) != "v03-fillnull-container-present" {
			t.Fatalf("fillnull parent rows = %#v", parentRows)
		}
		parentMap, ok := v03DynamicStringMap(parentRows[1][1])
		if !ok || v03JSONText(parentMap["child"]) != "<null>" ||
			v03JSONText(parentMap["sibling"]) != "keep-界" {
		...
		// v03JSONRows scans every physical driver column before selecting these
	...
	t.Run("addinfo overwrites authored shadows from immutable scope", func(t *testing.T) {
		const publicJobID = "v03-live-integration-job"
		compiled := v03CompileIntegrationSPLWithJobID(
			t,
		...
		rows := v03JSONRows(t, queryContext, connection, compiled,
		...
		if got := v03JSONText(rows[0][0]); got != fmt.Sprint(wantMin) {
			t.Fatalf("info_min_time = %s, want %d", got, wantMin)
		}
		if got := v03JSONText(rows[0][1]); got != fmt.Sprint(wantMax) {
			t.Fatalf("info_max_time = %s, want %d", got, wantMax)
		}
		if got := v03JSONText(rows[0][2]); got != fmt.Sprint(wantSearch) {
			t.Fatalf("info_search_time = %s, want %d", got, wantSearch)
		}
		if sid := v03JSONText(rows[0][3]); sid != publicJobID {
			t.Fatalf("info_sid = %q, want immutable admitted public job id %q", sid, publicJobID)
		}
	})

	t.Run("all ten commands execute together over hostile live data", func(t *testing.T) {
		const publicJobID = "v03-live-all-ten"
		compiled := v03CompileIntegrationSPLWithJobID(
			t,
	...
	t.Run("makemv Unicode empty-member and typed null boundaries", func(t *testing.T) {
		...
		v03AssertJSONRows(t, queryContext, connection, withoutEmpty,
		...
		v03AssertJSONRows(t, queryContext, connection, separatorBomb,
	...
	t.Run("mvexpand preserves member order scalar missing null and null members", func(t *testing.T) {
		...
		v03AssertJSONRows(t, queryContext, connection, expanded,
		...
		v03AssertJSONRows(t, queryContext, connection, numericScalar,
			[]string{"event_id", "n"}, [][]string{{"v03-01", "2"}})
		booleanScalar := compile(base + ` event_id="v03-03" | mvexpand n | table event_id n`)
		v03AssertJSONRows(t, queryContext, connection, booleanScalar,
			[]string{"event_id", "n"}, [][]string{{"v03-03", "true"}})
		v03AssertTaggedScalar(t, queryContext, connection, compile(
		...
		v03AssertJSONRows(t, queryContext, connection, repeated,
			[]string{"tags", "zones"},
			[][]string{
		...
		v03AssertJSONRows(t, queryContext, connection, reversed,
	...
	t.Run("multivalue hard ceilings fail atomically before downstream head", func(t *testing.T) {
		...
		v03AssertJSONRows(t, queryContext, connection, exactQueryRows,
...
func v03StoreIntegrationFixtureBatches(
	ctx context.Context,
	t *testing.T,
	store *Store,
	indexTime time.Time,
	...
	for start, batchNumber := 0, uint64(0); start < len(events); start, batchNumber = start+batchEvents, batchNumber+1 {
		...
		if _, err := store.Store(ctx, ingest.StoreBatch{
			...
			t.Fatalf("store v0.3 resource batch %s: %v", batchID, err)
		}
	}
}

func v03StoreDeltaBoundaryFixtureBatches(
	ctx context.Context,
	t *testing.T,
	store *Store,
	indexTime time.Time,
	...
	for start := 0; start < len(events); start += batchEvents {
		...
		if _, err := store.Store(ctx, ingest.StoreBatch{
			...
			t.Fatalf("store v0.3 delta boundary batch %s: %v", batchID, err)
		}
	}
}

func v03CompileIntegrationSPLWithJobID(
	t *testing.T,
	source string,
	cutoff time.Time,
...
func v03AssertTaggedScalar(
	...
	t.Helper()
	rows := v03JSONRows(t, ctx, connection, compiled, []string{"event_id", field})
...
func v03JSONRows(
	...
	for rows.Next() {
		...
		for fieldIndex, columnIndex := range requested {
			decoded[fieldIndex] = v03ScannedNativeValue(destinations[columnIndex])
		}
...
func v03ScannedNativeValue(destination any) any {
...
func v03DynamicStringMap(value any) (map[string]any, bool) {
	switch value := value.(type) {
	case chcol.Dynamic:
		if value.Nil() {
			return nil, false
		}
		return v03DynamicStringMap(value.Any())
	case *chcol.Dynamic:
...
func v03JSONText(value any) string {
	switch value := value.(type) {
	case nil:
		return "<null>"
	case chcol.Dynamic:
	...
	case []any:
		...
		for index, member := range value {
			parts[index] = v03JSONText(member)
		}
	...
	default:
		...
		if reflected.IsValid() && (reflected.Kind() == reflect.Array || reflected.Kind() == reflect.Slice) {
			...
			for index := 0; index < reflected.Len(); index++ {
				parts[index] = v03JSONText(reflected.Index(index).Interface())
			}
			return "[" + strings.Join(parts, ",") + "]"
		}
...
func v03AssertAtomicExecutionFailure(
	t *testing.T,
	ctx context.Context,
	connection clickhousedriver.Conn,
	compiled CompiledQuery,
	...
	if !compiled.RequiresAtomicResult() {
		t.Fatal("resource-sensitive v0.3 query did not retain atomic-result evidence")
	}
