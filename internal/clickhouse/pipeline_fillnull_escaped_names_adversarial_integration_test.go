...
const pipelineEscapedFillNullSource = "v03-fillnull-escaped-names"

func TestCompilePipelineFillNullEscapedPhysicalNameCompositions(t *testing.T) {
	t.Parallel()
...
func TestPipelineFillNullWeirdFieldAdmissionBoundary(t *testing.T) {
	t.Parallel()
	...
	// pipeline command fields are deliberately exact and unquoted. Quotes are not
...
// TestPipelineFillNullEscapedPhysicalNamesAgainstClickHouse pins the boundary
...
func TestPipelineFillNullEscapedPhysicalNamesAgainstClickHouse(t *testing.T) {
	...
	newEvent := func(
		...
		event.Event.Source = pipelineEscapedFillNullSource
	...
	base := `index=spl-v03-fillnull-escaped source="` +
		pipelineEscapedFillNullSource + `" | sort 0 +event_id`
	assertRows := func(source string, fields []string, want [][]string) {
		...
		pipelineAssertJSONRows(t, queryContext, connection, compiled, fields, want)
	}
