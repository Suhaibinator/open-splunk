...
func TestV03MVExpandPreservesFixedScalarOutputTypes(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			result, next, _, err := compileV03MVExpandTypeFixture(t, test.input)
			if err != nil {
...
func TestV03MVExpandKeepsDynamicOutputDynamic(t *testing.T) {
	t.Parallel()

	result, next, _, err := compileV03MVExpandTypeFixture(t, fieldState{
		valueSQL:       `"value"`,
...
func TestV03MVExpandPreservesFixedStringSemanticsAndRejectsNonText(t *testing.T) {
	t.Parallel()
	...
	t.Run("scalar", func(t *testing.T) {
		...
		result, next, _, err := compileV03MVExpandTypeFixture(t, input)
		if err != nil {
			t.Fatalf("compileExpandMultivalue: %v", err)
		}
		output := next.visible["value"]
...
func TestV03MVExpandDynamicStringBranchesRejectInvalidUTF8AndBytesProvenance(t *testing.T) {
	t.Parallel()

	result, _, _, err := compileV03MVExpandTypeFixture(t, fieldState{
		valueSQL:        `"value"`,
		dynamicTypeSQL:  `dynamicType("value")`,
		existsSQL:       "1",
		kind:            fieldKindDynamic,
...
func TestV03MakeMVRejectsNonTextAndPublishesTypedProvenance(t *testing.T) {
	t.Parallel()
	...
	result, next, _, err := compileV03MakeMVTypeFixture(t, input)
	if err != nil {
...
func TestV03MVExpandPreservesIndexCaseSensitiveComparison(t *testing.T) {
	t.Parallel()
...
func compileV03MVExpandTypeFixture(
	t *testing.T,
	input fieldState,
...
func compileV03MakeMVTypeFixture(
	t *testing.T,
	input fieldState,
) (compiledRelation, compileState, []any, error) {
	t.Helper()
