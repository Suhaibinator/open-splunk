...
func TestPipelineEveryCommandHasAnIndependentPhysicalLowering(t *testing.T) {
	t.Parallel()

	scope := testChartScope()
	scope.SearchJobID = "pipeline-independent-job"
	tests := []struct {
		name       string
		source     string
		wantFields []string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.bound != "" {
				...
				if !pipelineArgumentContainsString(compiled.Args, test.bound) {
...
func TestPipelineCommandsComposeWithoutLeakingAuthorityOrPrivateColumns(t *testing.T) {
	t.Parallel()

	const (
		jobID         = "pipeline/adversarial/job:秘密"
	...
	if !compiled.RequiresAtomicResult() {
		t.Fatal("resource-sensitive pipeline-command pipeline lacks atomic-result evidence")
	}
	...
	for _, value := range []struct {
		...
		if !pipelineArgumentContainsString(compiled.Args, value.bound) {
			t.Fatalf("bound arguments omit normalized value %q for authored %q: %#v", value.bound, value.authored, compiled.Args)
		}
	}
	if got := pipelineArgumentStringCount(compiled.Args, jobID); got != 1 {
		t.Fatalf("immutable info_sid occurs %d times in args, want exactly once: %#v", got, compiled.Args)
	}

	for _, marker := range pipelineAllRuntimeMarkers() {
		if !strings.Contains(compiled.SQL, marker) {
			t.Fatalf("pipeline-command pipeline omits runtime guard %q\nSQL: %s", marker, compiled.SQL)
		}
	}
	if !compiled.HasValidExecutionSeal() {
		t.Fatal("pipeline-command pipeline did not produce a valid sealed execution contract")
	}
}

func TestPipelineCommandsComposeWithExactRetainedKnowledgeEvidence(t *testing.T) {
	t.Parallel()

	program := testKnowledgeEvidenceProgram(t, "pipeline-all-commands-knowledge")
	scope := testChartScope()
	scope.SearchJobID = "pipeline-knowledge-job"
	...
	if err != nil {
		t.Fatalf("Compile(knowledge + pipeline commands) error = %v", err)
	}
	...
	if !ok {
		t.Fatal("compiled pipeline-command query omitted exact retained knowledge evidence")
	}
...
func TestPipelineFixedTypeBoundaryRejectsMakeMVButAdmitsScalarAndArrayMVExpand(t *testing.T) {
	t.Parallel()
...
func TestPipelineRepeatedExpansionAtStageBoundaryStaysBoundedAndPrivate(t *testing.T) {
	t.Parallel()
...
func TestPipelineRepeatedExpansionRetainsQueryWideChargeAcrossIntermediateOperators(t *testing.T) {
	t.Parallel()
...
func TestPipelineMVExpandGeneratedSQLHasBalancedStructuralParenthesesForEveryInputKind(t *testing.T) {
	t.Parallel()
	...
	for _, source := range []string{
		...
		t.Run(source, func(t *testing.T) {
			t.Parallel()
			compiled := compileSPL(t, source)
			pipelineAssertBalancedSQLParentheses(t, compiled.SQL)
		})
	}
}

func TestPipelineCompilerRevalidatesForgedPhaseTwoAndThreeOperators(t *testing.T) {
	t.Parallel()

	r := pipelineCompilerRange()
	field := func(name string) plan.FieldRef { return pipelineCompilerField(t, name, r) }
	...
	tests := []struct {
		...
		{name: "makemv forged private field", operators: []plan.Operator{&plan.MakeMultivalue{Input: pipelineForgedCompilerField("__os_private", r), Delimiter: ",", Range: r}}},
		...
		{name: "mvexpand forged private field", operators: []plan.Operator{&plan.ExpandMultivalue{Input: pipelineForgedCompilerField("__os_private", r), QueryOrdinal: 1, Range: r}}},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			query := pipelineForgedCompilerQuery(t, test.operators...)
...
func TestPipelineSourceDeterminedLimitsRemainResourceDiagnosticsAtCompilerBoundary(t *testing.T) {
	t.Parallel()

	r := pipelineCompilerRange()
	field := pipelineCompilerField(t, "n", r)
	tests := []struct {
		name     string
	...
	}{
		{
			...
			operator: &plan.FillNull{
				Fields: func() []plan.FieldRef {
					...
					for index := range result {
						result[index] = pipelineCompilerField(t, "overflow"+string(rune('a'+index%26))+string(rune('a'+index/26)), r)
					}
					return result
				}(),
				Value: "0",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			_, err := (Compiler{}).Compile(pipelineForgedCompilerQuery(t, test.operator))
...
func TestPipelineStrcatInheritsTheSharedConcatenationOutputByteCeiling(t *testing.T) {
	t.Parallel()
...
func pipelineAllRuntimeMarkers() []string {
	return []string{
...
func pipelineArgumentContainsString(arguments []any, want string) bool {
	return pipelineArgumentStringCount(arguments, want) > 0
}

func pipelineArgumentStringCount(arguments []any, want string) int {
	count := 0
...
func pipelineAssertBalancedSQLParentheses(t *testing.T, sql string) {
	t.Helper()
	depth := 0
	var quote byte
...
func pipelineCompilerRange() spl.Range {
	return spl.Range{
...
func pipelineCompilerField(t *testing.T, name string, sourceRange spl.Range) plan.FieldRef {
	t.Helper()
...
func pipelineForgedCompilerField(name string, sourceRange spl.Range) plan.FieldRef {
	return plan.FieldRef{Name: name, Path: []string{name}, Range: sourceRange}
}

func pipelineForgedCompilerQuery(t *testing.T, operators ...plan.Operator) *plan.Query {
	t.Helper()
