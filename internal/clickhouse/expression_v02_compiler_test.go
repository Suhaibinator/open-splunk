...
func TestCompileArithmeticV02NormalizesFixedOperandsAndOperators(t *testing.T) {
	t.Parallel()
		...
		t.Run(test.name, func(t *testing.T) {
			...
			compiled, err := compileScalarValue(
				&plan.ScalarBinaryExpression{
					Op:    test.op,
					Left:  expressionV02IntLiteral(5),
					Right: expressionV02FloatLiteral(-2),
				},
				state,
			)
			if err != nil {
...
func TestCompileArithmeticV02DynamicInputIsBoundAndBoundedOnce(t *testing.T) {
	t.Parallel()

	const source = "dynamic_arithmetic_source"
	state := expressionV02CompilerTestState(map[string]fieldState{
		"value": {
			valueSQL:       source,
...
func TestCompileArithmeticV02RejectsFixedTypes(t *testing.T) {
	t.Parallel()
	...
	_, err := compileScalarValue(
		&plan.ScalarBinaryExpression{
			...
			Left:  expressionV02StringLiteral("12"),
			Right: expressionV02IntLiteral(1),
		},
		state,
	)
	var diagnostic *plan.Diagnostic
...
func TestCompileMembershipV02BindsEveryOperandAndUsesExplicitThreeValuedLogic(t *testing.T) {
	t.Parallel()

	state := expressionV02CompilerTestState(nil)
	expression := &plan.MembershipExpression{
		Value: expressionV02IntLiteral(2),
		Candidates: []plan.ScalarExpression{
			expressionV02IntLiteral(1),
			expressionV02IntLiteral(2),
...
func TestCompileMembershipV02EveryPredicateConsumer(t *testing.T) {
	t.Parallel()
...
func TestCompileMembershipV02DynamicOperandValidatesMalformedTagsOnce(t *testing.T) {
	t.Parallel()
...
func TestCompileMembershipV02BindsOnlyRequiredDynamicClassifiers(t *testing.T) {
	t.Parallel()
	...
	numeric := compile(t, expressionV02IntLiteral(2))
	for _, required := range []string{
...
func TestEvalComparisonV02AppliesExplicitNaNRulesWithoutDuplicatingOperands(t *testing.T) {
	t.Parallel()
...
func TestExpressionV02RoundingPreservesIEEEComparisonSemantics(t *testing.T) {
	t.Parallel()
...
func TestCompileExpressionV02DefensiveBudgets(t *testing.T) {
	t.Parallel()
	...
	for range 256 {
		if _, err := compileScalarValue(
			&plan.ScalarUnaryExpression{
				Op:      plan.ScalarUnaryOpPositive,
				Operand: expressionV02IntLiteral(1),
			},
			state,
		); err != nil {
	...
	assertExpressionV02ComplexityError(t, err)

	membershipState := expressionV02CompilerTestState(nil)
	for range 8 {
		candidates := make([]plan.ScalarExpression, 32)
		for index := range candidates {
			candidates[index] = expressionV02IntLiteral(int64(index))
		}
		if _, _, err := compileExpression(
			&plan.MembershipExpression{
				Value:      expressionV02IntLiteral(0),
				Candidates: candidates,
			},
			membershipState,
		); err != nil {
	...
	_, _, err = compileExpression(
		&plan.MembershipExpression{
			Value:      expressionV02IntLiteral(0),
			Candidates: []plan.ScalarExpression{expressionV02IntLiteral(1)},
		},
		membershipState,
	)
	assertExpressionV02ComplexityError(t, err)
...
func TestCompileExpressionV02ParserPlanCompilerPipeline(t *testing.T) {
	t.Parallel()
...
func TestCompileExpressionV02MaximumAuthoredShapesStayWithinSQLBudget(t *testing.T) {
	...
	for _, fixture := range []struct {
	...
	}{
		{
			...
			source: expressionV02ArithmeticBenchmarkSource("severity", 256),
		},
		{
			name:   "Dynamic arithmetic 256",
			source: expressionV02ArithmeticBenchmarkSource("duration_ms", 256),
		},
		{
			name:   "membership 32",
...
func assertExpressionV02ComplexityError(t *testing.T, err error) {
	t.Helper()
	var diagnostic *plan.Diagnostic
...
func expressionV02CompilerTestState(fields map[string]fieldState) compileState {
	if fields == nil {
...
func expressionV02IntLiteral(value int64) plan.ScalarExpression {
	return &plan.ScalarLiteralExpression{
		Value: plan.Value{Kind: plan.ValueKindInt64, Int64: value},
	}
}

func expressionV02FloatLiteral(value float64) plan.ScalarExpression {
	return &plan.ScalarLiteralExpression{
		Value: plan.Value{Kind: plan.ValueKindFloat64, Float64: value},
	}
}

func expressionV02StringLiteral(value string) plan.ScalarExpression {
	return &plan.ScalarLiteralExpression{
		Value: plan.Value{Kind: plan.ValueKindString, String: value, Quoted: true},
	}
}

func expressionV02BoolLiteral(value bool) plan.ScalarExpression {
	return &plan.ScalarLiteralExpression{
